...
# # --- Additional Button for Recommended Products ---
# if st.button("Recommended Products"):
#     recommendations = fetch_recommendations()
#     if recommendations:
...
#         return similar_products.get("items", [])
#     except requests.exceptions.RequestException as e:
...
#                     <strong>Name:</strong> {item['product_name_en']}<br>
#                     <strong>Price:</strong> {item['price']}
#                     <button onclick="fetchSimilarProducts('{item['product_id']}')">Get Similar Products</button>
#                 </div>
#             </div>
#             """, unsafe_allow_html=True)

# # --- Button to Fetch Similar Products ---
# if st.button(t["fetch_similar_products"]):
#     product_id = "80957603-6cec-4f63-8dcd-aba6ded5cbdd"  # Example product ID
#     similar_products = fetch_similar_products(product_id)
#     if similar_products:
#         st.subheader(t["similar_products"])
...
#     /* Style for the similar products button */
...
#                     <button class="similar-products-button" onclick="fetchSimilarProducts('{product_id}')">Get Similar Products</button>
#                 </div>
#             </div>
#             """, unsafe_allow_html=True)
...
#     fetch(`/api/v1/ecommerce/recommendations/products/${productId}`)
#         .then(response => response.json())
#         .then(data => {
#             const similarProducts = data.items || [];
#             let similarProductsHTML = '<div class="chat-container bot"><div class="chat-icon bot-icon"></div><div class="chat-bubble bot-bubble"><strong>Similar Products:</strong><br>';
#             similarProducts.forEach(item => {
#                 similarProductsHTML += `<strong>Name:</strong> ${item.product_name_en}<br><strong>Price:</strong> ${item.price}<br>`;
#             });
#             similarProductsHTML += '</div></div>';
#             document.body.insertAdjacentHTML('beforeend', similarProductsHTML);
#         })
...
if st.button(t["fetch_recommendations"]):
    ...
    if recommendations:
        ...
        for item in recommendations:
            product_id = item['product_id']
            if st.button(f"Get Similar Products for {item['product_name_en']}", key=f"similar_{product_id}"):
                similar_products = fetch_similar_products(product_id)
                if similar_products:
                    st.subheader(t["similar_products"])
                    for similar_item in similar_products:
                        ...
                        <div class="product-card">
                            <strong>Name:</strong> {similar_item['product_name_en']}<br>
                            <strong>Price:</strong> {similar_item['price']}
                        </div>
                        """, unsafe_allow_html=True)
