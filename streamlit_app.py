# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "chat_title": "### 💬 Chat dengan PalmPal",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "#### 📌 Contoh Pertanyaan",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "👤 Kamu",
#         "bot": "🤖 PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "chat_title": "### 💬 Chat with PalmPal",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "#### 📌 Example Questions",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "👤 You",
#         "bot": "🤖 PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=200)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat History ---
# st.markdown(t['chat_title'])
# with st.container():
#     for msg in st.session_state.messages[-6:]:
#         speaker = t["user"] if msg["role"] == "user" else t["bot"]
#         st.markdown(f"**{speaker}:** {msg['content']}")

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""

# st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# st.button(t["send_button"], on_click=process_user_input)

# # --- Example Prompts ---
# st.markdown(t["examples"])
# cols = st.columns(2)
# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }
# for i, prompt in enumerate(example_prompts[lang]):
#     if cols[i % 2].button(prompt):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])
















# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS for Chat Bubbles ---
# st.markdown("""
# <style>
#     /* Chat container */
#     .chat-container {
#         padding: 10px;
#         border-radius: 10px;
#         margin-bottom: 10px;
#         position: relative;
#         max-width: 80%;
#         clear: both;
#     }
    
#     /* User message */
#     .user-message {
#         background-color: #e6f7ff;
#         border: 1px solid #91d5ff;
#         float: right;
#         color: #0c5460;
#         margin-left: 20%;
#     }
    
#     /* Bot message */
#     .bot-message {
#         background-color: #f0f2f5;
#         border: 1px solid #d9d9d9;
#         float: left;
#         color: #333;
#         margin-right: 20%;
#     }
    
#     /* Message content */
#     .message-content {
#         padding: 8px 12px;
#         font-size: 16px;
#     }
    
#     /* Avatar circle */
#     .avatar {
#         width: 32px;
#         height: 32px;
#         border-radius: 50%;
#         display: flex;
#         align-items: center;
#         justify-content: center;
#         color: white;
#         font-weight: bold;
#         position: absolute;
#         top: -5px;
#     }
    
#     /* User avatar */
#     .user-avatar {
#         background-color: #1890ff;
#         right: -10px;
#     }
    
#     /* Bot avatar */
#     .bot-avatar {
#         background-color: #52c41a;
#         left: -10px;
#     }
    
#     /* Clear float */
#     .clearfix::after {
#         content: "";
#         clear: both;
#         display: table;
#     }
    
#     /* Space between messages */
#     .message-gap {
#         height: 15px;
#         clear: both;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "chat_title": "### 💬 Chat dengan PalmPal",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "#### 📌 Contoh Pertanyaan",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "chat_title": "### 💬 Chat with PalmPal",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "#### 📌 Example Questions",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=200)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles ---
# st.markdown(t['chat_title'])
# chat_container = st.container()

# with chat_container:
#     for msg in st.session_state.messages[-6:]:
#         if msg["role"] == "user":
#             st.markdown(f"""
#             <div class="chat-container user-message">
#                 <div class="avatar user-avatar">👤</div>
#                 <div class="message-content">{msg['content']}</div>
#             </div>
#             <div class="message-gap"></div>
#             """, unsafe_allow_html=True)
#         else:
#             st.markdown(f"""
#             <div class="chat-container bot-message">
#                 <div class="avatar bot-avatar">🤖</div>
#                 <div class="message-content">{msg['content']}</div>
#             </div>
#             <div class="message-gap"></div>
#             """, unsafe_allow_html=True)
    
#     # Add clearfix to ensure container expands properly
#     st.markdown('<div class="clearfix"></div>', unsafe_allow_html=True)
# # --- Example Prompts ---
# st.markdown(t["examples"])
# cols = st.columns(2)
# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }
# for i, prompt in enumerate(example_prompts[lang]):
#     if cols[i % 2].button(prompt):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""

# input_col, button_col = st.columns([4, 1])
# with input_col:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with button_col:
#     st.button(t["send_button"], on_click=process_user_input)


# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])


















# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS for Chat Bubbles with SawitPRO colors ---
# st.markdown("""
# <style>
#     /* SawitPRO Colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-light-green: #8BA262;
#         --sawit-dark-green: #3A4A1F;
#         --sawit-light-yellow: #F5E685;
#     }
    
#     /* Add styling to fix button alignment */
#     .stButton > button {
#         height: 38px;
#         margin-top: 25px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         border: none !important;
#     }
    
#     .stButton > button:hover {
#         background-color: var(--sawit-dark-green) !important;
#         border: none !important;
#     }
    
#     /* Override Streamlit's default styling */
#     .stTextInput > div > div > input {
#         border: 1px solid var(--sawit-green);
#     }
    
#     /* Chat container */
#     .chat-container {
#         padding: 10px;
#         border-radius: 10px;
#         margin-bottom: 10px;
#         position: relative;
#         max-width: 80%;
#         clear: both;
#     }
    
#     /* User message */
#     .user-message {
#         background-color: var(--sawit-light-yellow);
#         border: 1px solid var(--sawit-yellow);
#         float: right;
#         color: #333;
#         margin-left: 20%;
#     }
    
#     /* Bot message */
#     .bot-message {
#         background-color: var(--sawit-light-green);
#         border: 1px solid var(--sawit-green);
#         float: left;
#         color: white;
#         margin-right: 20%;
#     }
    
#     /* Message content */
#     .message-content {
#         padding: 8px 12px;
#         font-size: 16px;
#     }
    
#     /* Avatar circle */
#     .avatar {
#         width: 32px;
#         height: 32px;
#         border-radius: 50%;
#         display: flex;
#         align-items: center;
#         justify-content: center;
#         color: white;
#         font-weight: bold;
#         position: absolute;
#         top: -5px;
#     }
    
#     /* User avatar */
#     .user-avatar {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         right: -10px;
#     }
    
#     /* Bot avatar */
#     .bot-avatar {
#         background-color: var(--sawit-green);
#         left: -10px;
#     }
    
#     /* Clear float */
#     .clearfix::after {
#         content: "";
#         clear: both;
#         display: table;
#     }
    
#     /* Space between messages */
#     .message-gap {
#         height: 15px;
#         clear: both;
#     }
    
#     /* Title styling */
#     h2, h3, h4 {
#         color: var(--sawit-green) !important;
#     }
    
#     /* Example buttons */
#     .examples-container button {
#         background-color: var(--sawit-light-green) !important;
#         color: white !important;
#         border: none !important;
#     }
    
#     .examples-container button:hover {
#         background-color: var(--sawit-green) !important;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "chat_title": "### 💬 Chat dengan PalmPal",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "#### 📌 Contoh Pertanyaan",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "chat_title": "### 💬 Chat with PalmPal",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "#### 📌 Example Questions",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=200)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles ---
# st.markdown(t['chat_title'])
# chat_container = st.container()

# with chat_container:
#     for msg in st.session_state.messages[-6:]:
#         if msg["role"] == "user":
#             st.markdown(f"""
#             <div class="chat-container user-message">
#                 <div class="avatar user-avatar">👤</div>
#                 <div class="message-content">{msg['content']}</div>
#             </div>
#             <div class="message-gap"></div>
#             """, unsafe_allow_html=True)
#         else:
#             st.markdown(f"""
#             <div class="chat-container bot-message">
#                 <div class="avatar bot-avatar">🤖</div>
#                 <div class="message-content">{msg['content']}</div>
#             </div>
#             <div class="message-gap"></div>
#             """, unsafe_allow_html=True)
    
#     # Add clearfix to ensure container expands properly
#     st.markdown('<div class="clearfix"></div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""

# # Create an input container with columns that align correctly
# input_container = st.container()
# with input_container:
#     cols = st.columns([4, 1])
#     with cols[0]:
#         st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
#     with cols[1]:
#         st.button(t["send_button"], on_click=process_user_input)

# # --- Example Prompts ---
# st.markdown(t["examples"])
# st.markdown('<div class="examples-container">', unsafe_allow_html=True)
# cols = st.columns(2)
# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }
# for i, prompt in enumerate(example_prompts[lang]):
#     if cols[i % 2].button(prompt):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])













# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Chat bubbles with icons */
#     .chat-container {
#         display: flex;
#         margin-bottom: 15px;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 40px;
#         height: 40px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 10px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 10px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 12px 16px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Fix button alignment */
#     div.stButton > button {
#         margin-top: 24px;
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Example prompt styling */
#     .example-section {
#         margin-bottom: 20px;
#     }
    
#     .example-section button {
#         width: 100%;
#         margin-bottom: 10px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         opacity: 0.9;
#     }
    
#     .example-section button:hover {
#         opacity: 1;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Create space at bottom of chat container */
#     .chat-end-spacer {
#         height: 20px;
#         clear: both;
#     }
    
#     /* Chat container with fixed height and scrolling */
#     .chat-window {
#         height: 400px;
#         overflow-y: auto;
#         border: 1px solid #eee;
#         border-radius: 10px;
#         padding: 15px;
#         margin-bottom: 20px;
#         background-color: #f9f9f9;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "chat_title": "### 💬 Chat dengan PalmPal",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "#### 📌 Contoh Pertanyaan",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "chat_title": "### 💬 Chat with PalmPal",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "#### 📌 Example Questions",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=200)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Example Prompts (MOVED ABOVE INPUT) ---
# st.markdown(t["examples"])
# st.markdown('<div class="example-section">', unsafe_allow_html=True)
# example_cols = st.columns(2)
# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# for i, prompt in enumerate(example_prompts[lang]):
#     if example_cols[i % 2].button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Display Chat Bubbles ---
# st.markdown(t['chat_title'])
# chat_container = st.container()

# with chat_container:
#     # Create a fixed-height chat window
#     st.markdown('<div class="chat-window">', unsafe_allow_html=True)
    
#     # Display chat messages with user/bot icons
#     for msg in st.session_state.messages[-6:]:
#         if msg["role"] == "user":
#             st.markdown(f"""
#             <div class="chat-container user">
#                 <div class="chat-bubble user-bubble">{msg['content']}</div>
#                 <div class="chat-icon user-icon"></div>
#             </div>
#             """, unsafe_allow_html=True)
#         else:
#             st.markdown(f"""
#             <div class="chat-container bot">
#                 <div class="chat-icon bot-icon"></div>
#                 <div class="chat-bubble bot-bubble">{msg['content']}</div>
#             </div>
#             """, unsafe_allow_html=True)
    
#     # Close the chat window
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.button(t["send_button"], on_click=process_user_input)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])























# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Chat bubbles with icons */
#     .chat-container {
#         display: flex;
#         margin-bottom: 12px;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 12px 16px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Chat frame */
#     .chat-frame {
#         border: 2px solid #e0e0e0;
#         border-radius: 10px;
#         padding: 15px;
#         margin-bottom: 20px;
#         background-color: #f9f9f9;
#         height: 400px;
#         overflow-y: auto;
#     }
    
#     /* Fix button alignment */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Example prompts styling */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 15px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 8px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "chat_title": "### 💬 Chat dengan PalmPal",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "chat_title": "### 💬 Chat with PalmPal",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=200)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles with Frame ---
# st.markdown(t['chat_title'])

# # Ensure the chat frame has a proper height and scrolling
# st.markdown('<div class="chat-frame">', unsafe_allow_html=True)
    
# # Display chat messages with user/bot icons
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # Close the chat frame
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Example Prompts RIGHT ABOVE INPUT ---
# st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# # Custom HTML grid for examples with equal spacing
# st.markdown('<div class="example-grid">', unsafe_allow_html=True)
# for i, prompt in enumerate(example_prompts[lang]):
#     if st.button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])








# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Chat bubbles with icons */
#     .chat-container {
#         display: flex;
#         margin-bottom: 12px;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 12px 16px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Chat frame */
#     .chat-frame {
#         border: 2px solid #e0e0e0;
#         border-radius: 10px;
#         padding: 15px;
#         margin-bottom: 20px;
#         background-color: #f9f9f9;
#         height: 400px;
#         overflow-y: auto;
#     }
    
#     /* Fix button alignment */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Example prompts styling */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 15px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 8px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=200)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles with Frame ---
# # Removed the chat title

# # Ensure the chat frame has a proper height and scrolling
# st.markdown('<div class="chat-frame">', unsafe_allow_html=True)
    
# # Display chat messages with user/bot icons
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # Close the chat frame
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Example Prompts RIGHT ABOVE INPUT ---
# st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# # Custom HTML grid for examples with equal spacing
# st.markdown('<div class="example-grid">', unsafe_allow_html=True)
# for i, prompt in enumerate(example_prompts[lang]):
#     if st.button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])

# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Chat bubbles with icons */
#     .chat-container {
#         display: flex;
#         margin-bottom: 12px;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 12px 16px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* No more chat frame - messages will appear directly on page */
#     .chat-area {
#         padding: 15px 5px;
#         margin-bottom: 20px;
#         height: 400px;
#         overflow-y: auto;
#     }
    
#     /* Fix button alignment */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Example prompts styling */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 15px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 8px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=200)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles (No Frame) ---
# # Using a regular container without the frame styling
# st.markdown('<div class="chat-area">', unsafe_allow_html=True)
    
# # Display chat messages with user/bot icons
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # Close the chat area
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Example Prompts RIGHT ABOVE INPUT ---
# st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# # Custom HTML grid for examples with equal spacing
# st.markdown('<div class="example-grid">', unsafe_allow_html=True)
# for i, prompt in enumerate(example_prompts[lang]):
#     if st.button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])











# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Remove default Streamlit padding and margins */
#     .reportview-container .main .block-container {
#         padding-top: 1rem;
#         padding-right: 1rem;
#         padding-left: 1rem;
#         padding-bottom: 1rem;
#         margin-top: -60px;
#     }
    
#     /* Streamlit header adjustments to reduce space */
#     .stApp header {
#         height: 0px !important;
#         padding: 0 !important;
#         margin: 0 !important;
#     }
    
#     /* Reduce space around image and title */
#     img {
#         margin-bottom: 0.5rem !important;
#     }
    
#     h2 {
#         margin-top: 0 !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#     }
    
#     /* Reduce space for subtitle */
#     p {
#         margin-bottom: 0.5rem !important;
#     }
    
#     /* Chat bubbles with icons */
#     .chat-container {
#         display: flex;
#         margin-bottom: 10px;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 12px 16px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Compact chat area with reduced padding */
#     .chat-area {
#         padding: 0;
#         margin-bottom: 10px;
#         height: auto;
#         min-height: 100px;
#     }
    
#     /* Fix button alignment */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Example prompts styling - more compact */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 10px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#     }
    
#     /* Example section header - more compact */
#     .example-header {
#         margin-bottom: 5px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Reduce space in footer */
#     footer {
#         margin-top: 1rem !important;
#     }
    
#     /* Reduce space between elements */
#     .block-container > div > div {
#         gap: 0.5rem !important;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Language Toggle Button ---
# lang_toggle = st.button("🌐 Switch to English" if st.session_state.language == "id" else "🌐 Ganti ke Bahasa Indonesia")
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()  # Force rerun to reflect UI change immediately

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan."
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error."
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Only use one st.image with smaller dimensions ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=150)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles (No Frame, Compact) ---
# # No container div around this section to reduce space
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # --- Example Prompts RIGHT ABOVE INPUT (Compact) ---
# st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# # Custom HTML grid for examples with equal spacing
# st.markdown('<div class="example-grid">', unsafe_allow_html=True)
# for i, prompt in enumerate(example_prompts[lang]):
#     if st.button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])

# # --- Footer ---
# st.markdown("---")
# st.markdown(t["footer"])













# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Reduce space but not as extreme as before */
#     .block-container {
#         padding-top: 1rem !important;
#         padding-bottom: 0rem !important;
#         margin-top: -2rem !important;
#     }
    
#     /* Further reduce space in the main area */
#     .main .block-container {
#         padding: 1rem !important; 
#         max-width: 100% !important;
#     }
    
#     /* Reduce spaces and gaps - but leave some breathing room */
#     .css-1y4p8pa {
#         margin-top: -1rem !important;
#         padding-top: 0.5rem !important;
#         padding-bottom: 0rem !important;
#     }
    
#     /* Reduce space above the chat messages */
#     .css-1544g2n {
#         padding-top: 0.5rem !important;
#         margin-top: 0 !important;
#     }
    
#     /* Reduce any extra margins around elements */
#     .element-container, .stMarkdown, .stButton, .stImage {
#         margin-top: 0.2rem !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#         padding-bottom: 0 !important;
#     }
    
#     /* Reduce space around image */
#     img {
#         margin-bottom: 0.5rem !important;
#     }
    
#     /* Keep more breathing room in title and subtitle */
#     h2 {
#         margin-top: 0 !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#     }
    
#     p {
#         margin-bottom: 0.5rem !important;
#         padding: 0 !important;
#         line-height: 1.2 !important;
#     }
    
#     /* Chat bubbles with icons - slightly more room */
#     .chat-container {
#         display: flex;
#         margin-bottom: 8px !important;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 10px 14px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Restore SawitPRO brand colors for example prompts */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 8px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.95em !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 5px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Language toggle button styling */
#     .lang-toggle {
#         display: inline-block;
#         margin-bottom: 8px;
#     }
    
#     .lang-toggle button {
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         border: none !important;
#         font-size: 0.9em !important;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Add moderate padding to gently push things up */
#     .filler {
#         height: 1px;
#         margin-top: -150px !important;
#     }
    
#     /* Hide footer but keep header */
#     footer {
#         display: none !important;
#     }
    
#     /* Make textfields look better */
#     .stTextInput > div {
#         margin-top: 0 !important;
#         margin-bottom: 8px !important;
#     }
    
#     /* Custom styling for buttons */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Add a filler div to push things up but not too aggressively ---
# st.markdown('<div class="filler"></div>', unsafe_allow_html=True)

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan.",
#         "lang_toggle": "🌐 Switch to English"
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error.",
#         "lang_toggle": "🌐 Ganti ke Bahasa Indonesia"
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Restored Language Toggle Button ---
# st.markdown('<div class="lang-toggle">', unsafe_allow_html=True)
# lang_toggle = st.button(t["lang_toggle"])
# if lang_toggle:
#     st.session_state.language = "en" if st.session_state.language == "id" else "id"
#     st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Only use one st.image with smaller dimensions ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=120)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles ---
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # --- Example Prompts RIGHT ABOVE INPUT ---
# st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# # Custom HTML grid for examples with equal spacing
# st.markdown('<div class="example-grid">', unsafe_allow_html=True)
# for i, prompt in enumerate(example_prompts[lang]):
#     if st.button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []




























# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Reduce space but not as extreme as before */
#     .block-container {
#         padding-top: 1rem !important;
#         padding-bottom: 0rem !important;
#         margin-top: -2rem !important;
#     }
    
#     /* Further reduce space in the main area */
#     .main .block-container {
#         padding: 1rem !important; 
#         max-width: 100% !important;
#     }
    
#     /* Reduce spaces and gaps - but leave some breathing room */
#     .css-1y4p8pa {
#         margin-top: -1rem !important;
#         padding-top: 0.5rem !important;
#         padding-bottom: 0rem !important;
#     }
    
#     /* Reduce space above the chat messages */
#     .css-1544g2n {
#         padding-top: 0.5rem !important;
#         margin-top: 0 !important;
#     }
    
#     /* Reduce any extra margins around elements */
#     .element-container, .stMarkdown, .stButton, .stImage {
#         margin-top: 0.2rem !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#         padding-bottom: 0 !important;
#     }
    
#     /* Reduce space around image */
#     img {
#         margin-bottom: 0.5rem !important;
#     }
    
#     /* Keep more breathing room in title and subtitle */
#     h2 {
#         margin-top: 0 !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#     }
    
#     p {
#         margin-bottom: 0.5rem !important;
#         padding: 0 !important;
#         line-height: 1.2 !important;
#     }
    
#     /* Chat bubbles with icons - slightly more room */
#     .chat-container {
#         display: flex;
#         margin-bottom: 8px !important;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 10px 14px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Restore SawitPRO brand colors for example prompts */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 8px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.95em !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 5px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Language toggle button styling */
#     .lang-toggle {
#         display: inline-block;
#         margin-bottom: 8px;
#     }
    
#     .lang-toggle button {
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         border: none !important;
#         font-size: 0.9em !important;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Add moderate padding to gently push things up */
#     .filler {
#         height: 1px;
#         margin-top: -150px !important;
#     }
    
#     /* Hide footer but keep header */
#     footer {
#         display: none !important;
#     }
    
#     /* Make textfields look better */
#     .stTextInput > div {
#         margin-top: 0 !important;
#         margin-bottom: 8px !important;
#     }
    
#     /* Custom styling for buttons */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Add a filler div to push things up but not too aggressively ---
# st.markdown('<div class="filler"></div>', unsafe_allow_html=True)

# # --- Translation function for existing messages ---
# def translate_content(text, from_lang, to_lang):
#     """
#     Simple translation function using keyword mapping.
#     In a production environment, you might want to use a more sophisticated translation service.
#     """
#     if from_lang == to_lang:
#         return text
    
#     # Define translation mappings
#     translations = {
#         ("id", "en"): {
#             "Kenapa daun sawit saya kuning?": "Why are my palm leaves yellow?",
#             "Bagaimana cara pupuk sawit yang bagus?": "What's a good way to fertilize palm oil trees?",
#             "Apa hama yang sering menyerang sawit?": "What pests commonly attack palm oil plants?",
#             "Bagaimana panen sawit yang efektif?": "How to harvest palm oil effectively?",
#             "Ada beberapa alasan": "There are several reasons",
#             "mengapa daun sawit": "why palm leaves",
#             "menguning": "turn yellow",
#             "Berikut beberapa": "Here are some",
#             "kemungkinan penyebabnya": "possible causes",
#             "Kelebihan air": "Overwatering",
#             "Kekurangan air": "Underwatering", 
#             "Kekurangan nutrisi": "Nutrient deficiency",
#             "Serangan hama": "Pest infestation",
#             "Fluktuasi suhu": "Temperature fluctuations",
#             "Untuk pemupukan sawit": "For palm fertilization",
#             "yang baik": "that is good",
#             "Hama umum": "Common pests",
#             "tanaman sawit": "palm plants",
#             "Panen sawit": "Palm harvesting",
#             "yang efektif": "that is effective"
#         },
#         ("en", "id"): {
#             "Why are my palm leaves yellow?": "Kenapa daun sawit saya kuning?",
#             "What's a good way to fertilize palm oil trees?": "Bagaimana cara pupuk sawit yang bagus?",
#             "What pests commonly attack palm oil plants?": "Apa hama yang sering menyerang sawit?",
#             "How to harvest palm oil effectively?": "Bagaimana panen sawit yang efektif?",
#             "There are several reasons": "Ada beberapa alasan",
#             "why palm leaves": "mengapa daun sawit",
#             "turn yellow": "menguning",
#             "Here are some": "Berikut beberapa",
#             "possible causes": "kemungkinan penyebabnya",
#             "Overwatering": "Kelebihan air",
#             "Underwatering": "Kekurangan air",
#             "Nutrient deficiency": "Kekurangan nutrisi", 
#             "Pest infestation": "Serangan hama",
#             "Temperature fluctuations": "Fluktuasi suhu",
#             "For palm fertilization": "Untuk pemupukan sawit",
#             "that is good": "yang baik",
#             "Common pests": "Hama umum",
#             "palm plants": "tanaman sawit",
#             "Palm harvesting": "Panen sawit",
#             "that is effective": "yang efektif"
#         }
#     }
    
#     # Get the appropriate translation dictionary
#     trans_dict = translations.get((from_lang, to_lang), {})
    
#     # Try exact match first
#     if text in trans_dict:
#         return trans_dict[text]
    
#     # Try partial translation for longer texts
#     translated_text = text
#     for original, translation in trans_dict.items():
#         if original in translated_text:
#             translated_text = translated_text.replace(original, translation)
    
#     return translated_text

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan.",
#         "lang_toggle": "🌐 Switch to English"
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error.",
#         "lang_toggle": "🌐 Ganti ke Bahasa Indonesia"
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Restored Language Toggle Button with Translation Feature ---
# st.markdown('<div class="lang-toggle">', unsafe_allow_html=True)
# lang_toggle = st.button(t["lang_toggle"])
# if lang_toggle:
#     # Store the old language before switching
#     old_lang = st.session_state.language
#     new_lang = "en" if st.session_state.language == "id" else "id"
    
#     # Translate existing messages when language is switched
#     if 'messages' in st.session_state and st.session_state.messages:
#         translated_messages = []
#         for message in st.session_state.messages:
#             translated_content = translate_content(message["content"], old_lang, new_lang)
#             translated_messages.append({
#                 "role": message["role"],
#                 "content": translated_content
#             })
#         st.session_state.messages = translated_messages
    
#     # Switch the language
#     st.session_state.language = new_lang
#     st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Only use one st.image with smaller dimensions ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=120)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles ---
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # --- Example Prompts RIGHT ABOVE INPUT ---
# st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# # Custom HTML grid for examples with equal spacing
# st.markdown('<div class="example-grid">', unsafe_allow_html=True)
# for i, prompt in enumerate(example_prompts[lang]):
#     if st.button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])
#         st.rerun()
#     st.markdown(t["model_info"])










# import streamlit as st
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Reduce space but not as extreme as before */
#     .block-container {
#         padding-top: 1rem !important;
#         padding-bottom: 0rem !important;
#         margin-top: -2rem !important;
#     }
    
#     /* Further reduce space in the main area */
#     .main .block-container {
#         padding: 1rem !important; 
#         max-width: 100% !important;
#     }
    
#     /* Reduce spaces and gaps - but leave some breathing room */
#     .css-1y4p8pa {
#         margin-top: -1rem !important;
#         padding-top: 0.5rem !important;
#         padding-bottom: 0rem !important;
#     }
    
#     /* Reduce space above the chat messages */
#     .css-1544g2n {
#         padding-top: 0.5rem !important;
#         margin-top: 0 !important;
#     }
    
#     /* Reduce any extra margins around elements */
#     .element-container, .stMarkdown, .stButton, .stImage {
#         margin-top: 0.2rem !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#         padding-bottom: 0 !important;
#     }
    
#     /* Reduce space around image */
#     img {
#         margin-bottom: 0.5rem !important;
#     }
    
#     /* Keep more breathing room in title and subtitle */
#     h2 {
#         margin-top: 0 !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#     }
    
#     p {
#         margin-bottom: 0.5rem !important;
#         padding: 0 !important;
#         line-height: 1.2 !important;
#     }
    
#     /* Chat bubbles with icons - slightly more room */
#     .chat-container {
#         display: flex;
#         margin-bottom: 8px !important;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 10px 14px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Restore SawitPRO brand colors for example prompts */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 8px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.95em !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 5px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Language toggle button styling */
#     .lang-toggle {
#         display: inline-block;
#         margin-bottom: 8px;
#     }
    
#     .lang-toggle button {
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         border: none !important;
#         font-size: 0.9em !important;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Add moderate padding to gently push things up */
#     .filler {
#         height: 1px;
#         margin-top: -150px !important;
#     }
    
#     /* Hide footer but keep header */
#     footer {
#         display: none !important;
#     }
    
#     /* Make textfields look better */
#     .stTextInput > div {
#         margin-top: 0 !important;
#         margin-bottom: 8px !important;
#     }
    
#     /* Custom styling for buttons */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Add a filler div to push things up but not too aggressively ---
# st.markdown('<div class="filler"></div>', unsafe_allow_html=True)

# # --- Translation function for existing messages ---
# def translate_content(text, from_lang, to_lang):
#     """
#     Simple translation function using keyword mapping.
#     In a production environment, you might want to use a more sophisticated translation service.
#     """
#     if from_lang == to_lang:
#         return text
    
#     # Define translation mappings with more comprehensive coverage
#     translations = {
#         ("id", "en"): {
#             "Kenapa daun sawit saya kuning?": "Why are my palm leaves yellow?",
#             "Bagaimana cara pupuk sawit yang bagus?": "What's a good way to fertilize palm oil trees?",
#             "Apa hama yang sering menyerang sawit?": "What pests commonly attack palm oil plants?",
#             "Bagaimana panen sawit yang efektif?": "How to harvest palm oil effectively?",
            
#             # Common response patterns
#             "Ada beberapa alasan mengapa daun sawit Anda menguning": "There are several reasons why your palm leaves might be turning yellow",
#             "Ada beberapa alasan": "There are several reasons",
#             "mengapa daun sawit": "why palm leaves",
#             "menguning": "turn yellow",
#             "Berikut beberapa kemungkinan penyebabnya": "Here are some possible causes",
#             "Berikut beberapa": "Here are some",
#             "kemungkinan penyebabnya": "possible causes",
#             "Kelebihan air": "Overwatering",
#             "Kekurangan air": "Underwatering", 
#             "Kekurangan nutrisi": "Nutrient deficiency",
#             "Serangan hama": "Pest infestation",
#             "Fluktuasi suhu": "Temperature fluctuations",
#             "Untuk pemupukan sawit": "For palm fertilization",
#             "yang baik": "that is good",
#             "Hama umum": "Common pests",
#             "tanaman sawit": "palm plants",
#             "Panen sawit": "Palm harvesting",
#             "yang efektif": "that is effective",
            
#             # More comprehensive translations
#             "Terlalu banyak air": "Too much water",
#             "dapat menyebabkan": "can cause",
#             "akar membusuk": "root rot",
#             "daun menguning": "yellowing leaves",
#             "Periksa kelembaban tanah": "Check soil moisture",
#             "tanah terlalu kering": "soil is too dry",
#             "membutuhkan air": "needs water",
#             "nutrisi penting": "important nutrients",
#             "nitrogen": "nitrogen",
#             "zat besi": "iron",
#             "magnesium": "magnesium",
#             "tetap sehat": "stay healthy",
#             "penyakit jamur": "fungal disease",
#             "suhu antara": "temperature between",
#             "terpapar suhu ekstrim": "exposed to extreme temperatures"
#         },
#         ("en", "id"): {
#             "Why are my palm leaves yellow?": "Kenapa daun sawit saya kuning?",
#             "What's a good way to fertilize palm oil trees?": "Bagaimana cara pupuk sawit yang bagus?",
#             "What pests commonly attack palm oil plants?": "Apa hama yang sering menyerang sawit?",
#             "How to harvest palm oil effectively?": "Bagaimana panen sawit yang efektif?",
            
#             # Common response patterns
#             "There are several reasons why your palm leaves might be turning yellow": "Ada beberapa alasan mengapa daun sawit Anda menguning",
#             "There are several reasons": "Ada beberapa alasan",
#             "why palm leaves": "mengapa daun sawit",
#             "turn yellow": "menguning",
#             "Here are some possible causes": "Berikut beberapa kemungkinan penyebabnya",
#             "Here are some": "Berikut beberapa",
#             "possible causes": "kemungkinan penyebabnya",
#             "Overwatering": "Kelebihan air",
#             "Underwatering": "Kekurangan air",
#             "Nutrient deficiency": "Kekurangan nutrisi", 
#             "Pest infestation": "Serangan hama",
#             "Temperature fluctuations": "Fluktuasi suhu",
#             "For palm fertilization": "Untuk pemupukan sawit",
#             "that is good": "yang baik",
#             "Common pests": "Hama umum",
#             "palm plants": "tanaman sawit",
#             "Palm harvesting": "Panen sawit",
#             "that is effective": "yang efektif",
            
#             # More comprehensive translations
#             "Too much water": "Terlalu banyak air",
#             "can cause": "dapat menyebabkan",
#             "root rot": "akar membusuk",
#             "yellowing leaves": "daun menguning",
#             "Check soil moisture": "Periksa kelembaban tanah",
#             "soil is too dry": "tanah terlalu kering",
#             "needs water": "membutuhkan air",
#             "important nutrients": "nutrisi penting",
#             "nitrogen": "nitrogen",
#             "iron": "zat besi",
#             "magnesium": "magnesium",
#             "stay healthy": "tetap sehat",
#             "fungal disease": "penyakit jamur",
#             "temperature between": "suhu antara",
#             "exposed to extreme temperatures": "terpapar suhu ekstrim"
#         }
#     }
    
#     # Get the appropriate translation dictionary
#     trans_dict = translations.get((from_lang, to_lang), {})
    
#     # Try exact match first
#     if text.strip() in trans_dict:
#         return trans_dict[text.strip()]
    
#     # Try partial translation for longer texts
#     translated_text = text
#     # Sort by length (longest first) to avoid partial replacements
#     sorted_translations = sorted(trans_dict.items(), key=lambda x: len(x[0]), reverse=True)
    
#     for original, translation in sorted_translations:
#         if original.lower() in translated_text.lower():
#             # Case-insensitive replacement
#             import re
#             pattern = re.compile(re.escape(original), re.IGNORECASE)
#             translated_text = pattern.sub(translation, translated_text)
    
#     return translated_text

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan.",
#         "lang_toggle": "🌐 Switch to English"
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error.",
#         "lang_toggle": "🌐 Ganti ke Bahasa Indonesia"
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Restored Language Toggle Button with Translation Feature ---
# st.markdown('<div class="lang-toggle">', unsafe_allow_html=True)
# lang_toggle = st.button(t["lang_toggle"])
# if lang_toggle:
#     # Store the old language before switching
#     old_lang = st.session_state.language
#     new_lang = "en" if st.session_state.language == "id" else "id"
    
#     # Debug: Show current state
#     st.write(f"Debug: Switching from {old_lang} to {new_lang}")
#     st.write(f"Debug: Current messages count: {len(st.session_state.get('messages', []))}")
    
#     # Translate existing messages when language is switched
#     if 'messages' in st.session_state and st.session_state.messages:
#         translated_messages = []
#         for i, message in enumerate(st.session_state.messages):
#             original_content = message["content"]
#             translated_content = translate_content(original_content, old_lang, new_lang)
#             translated_messages.append({
#                 "role": message["role"],
#                 "content": translated_content
#             })
#             # Debug: Show translation
#             st.write(f"Debug {i}: '{original_content}' -> '{translated_content}'")
        
#         st.session_state.messages = translated_messages
#         st.write(f"Debug: Translation completed. New messages count: {len(st.session_state.messages)}")
    
#     # Switch the language
#     st.session_state.language = new_lang
#     st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Only use one st.image with smaller dimensions ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=120)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Display Chat Bubbles ---
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # --- Example Prompts RIGHT ABOVE INPUT ---
# st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

# example_prompts = {
#     "id": [
#         "Kenapa daun sawit saya kuning?",
#         "Bagaimana cara pupuk sawit yang bagus?",
#         "Apa hama yang sering menyerang sawit?",
#         "Bagaimana panen sawit yang efektif?"
#     ],
#     "en": [
#         "Why are my palm leaves yellow?",
#         "What's a good way to fertilize palm oil trees?",
#         "What pests commonly attack palm oil plants?",
#         "How to harvest palm oil effectively?"
#     ]
# }

# # Custom HTML grid for examples with equal spacing
# st.markdown('<div class="example-grid">', unsafe_allow_html=True)
# for i, prompt in enumerate(example_prompts[lang]):
#     if st.button(prompt, key=f"example_{i}"):
#         st.session_state.messages.append({"role": "user", "content": prompt})
#         bot_response = generate_response(prompt)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
#         bot_response = generate_response(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.rerun()
#     st.markdown(t["model_info"])












# BEFORE API !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


import streamlit as st
from huggingface_hub import InferenceClient

api_key = st.secrets["HUGGINGFACE_API_KEY"]
client = InferenceClient(api_key=api_key)

# --- Page Configuration ---
st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# --- Custom CSS with SawitPRO colors and icons ---
st.markdown("""
<style>
    /* Main colors */
    :root {
        --sawit-green: #576A34;
        --sawit-yellow: #E9D037;
        --sawit-dark-green: #3A4A1F;
    }
    
    /* Reduce space but not as extreme as before */
    .block-container {
        padding-top: 1rem !important;
        padding-bottom: 0rem !important;
        margin-top: -2rem !important;
    }
    
    /* Further reduce space in the main area */
    .main .block-container {
        padding: 1rem !important; 
        max-width: 100% !important;
    }
    
    /* Reduce spaces and gaps - but leave some breathing room */
    .css-1y4p8pa {
        margin-top: -1rem !important;
        padding-top: 0.5rem !important;
        padding-bottom: 0rem !important;
    }
    
    /* Reduce space above the chat messages */
    .css-1544g2n {
        padding-top: 0.5rem !important;
        margin-top: 0 !important;
    }
    
    /* Reduce any extra margins around elements */
    .element-container, .stMarkdown, .stButton, .stImage {
        margin-top: 0.2rem !important;
        margin-bottom: 0.2rem !important;
        padding-top: 0 !important;
        padding-bottom: 0 !important;
    }
    
    /* Reduce space around image */
    img {
        margin-bottom: 0.5rem !important;
    }
    
    /* Keep more breathing room in title and subtitle */
    h2 {
        margin-top: 0 !important;
        margin-bottom: 0.2rem !important;
        padding-top: 0 !important;
    }
    
    p {
        margin-bottom: 0.5rem !important;
        padding: 0 !important;
        line-height: 1.2 !important;
    }
    
    /* Chat bubbles with icons - slightly more room */
    .chat-container {
        display: flex;
        margin-bottom: 8px !important;
        max-width: 100%;
        clear: both;
    }
    
    .chat-container.user {
        justify-content: flex-end;
    }
    
    .chat-container.bot {
        justify-content: flex-start;
    }
    
    .chat-icon {
        width: 36px;
        height: 36px;
        border-radius: 50%;
        background-size: cover;
        background-position: center;
        flex-shrink: 0;
        margin-top: auto;
    }
    
    .user-icon {
        background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
        margin-left: 8px;
    }
    
    .bot-icon {
        background-image: url('data:image/png;base64,');
        margin-right: 8px;
    }
    
    .chat-bubble {
        border-radius: 18px;
        padding: 10px 14px;
        max-width: 70%;
        word-wrap: break-word;
    }
    
    .user-bubble {
        background-color: var(--sawit-yellow);
        color: #333;
        border-top-right-radius: 4px;
    }
    
    .bot-bubble {
        background-color: var(--sawit-green);
        color: white;
        border-top-left-radius: 4px;
    }
    
    /* Restore SawitPRO brand colors for example prompts */
    .example-grid {
        display: grid;
        grid-template-columns: 1fr 1fr;
        grid-gap: 8px;
        margin-bottom: 8px;
    }
    
    .example-grid button {
        width: 100%;
        height: 100%;
        padding: 8px;
        background-color: var(--sawit-green) !important;
        color: white !important;
        font-size: 0.95em !important;
    }
    
    /* Example section header */
    .example-header {
        margin-bottom: 5px;
        color: var(--sawit-green);
        font-weight: bold;
    }
    
    /* Language toggle button styling */
    .lang-toggle {
        display: inline-block;
        margin-bottom: 8px;
    }
    
    .lang-toggle button {
        background-color: var(--sawit-green) !important;
        color: white !important;
        border: none !important;
        font-size: 0.9em !important;
    }
    
    /* Align send button */
    .send-button button {
        margin-top: 24px;
    }
    
    /* Header colors */
    h1, h2, h3, h4, h5, h6 {
        color: var(--sawit-green);
    }
    
    /* Add moderate padding to gently push things up */
    .filler {
        height: 1px;
        margin-top: -150px !important;
    }
    
    /* Hide footer but keep header */
    footer {
        display: none !important;
    }
    
    /* Make textfields look better */
    .stTextInput > div {
        margin-top: 0 !important;
        margin-bottom: 8px !important;
    }
    
    /* Custom styling for buttons */
    div.stButton > button {
        background-color: var(--sawit-green);
        color: white;
        border: none;
    }
    
    div.stButton > button:hover {
        background-color: var(--sawit-dark-green);
        color: white;
        border: none;
    }
</style>
""", unsafe_allow_html=True)

# --- Add a filler div to push things up but not too aggressively ---
st.markdown('<div class="filler"></div>', unsafe_allow_html=True)

# --- Translation function for existing messages ---
def translate_content(text, from_lang, to_lang):
    """
    Translation function that uses both keyword mapping and AI translation.
    """
    if from_lang == to_lang:
        return text
    
    # Define translation mappings for quick common phrases
    translations = {
        ("id", "en"): {
            "Kenapa daun sawit saya kuning?": "Why are my palm leaves yellow?",
            "Bagaimana cara pupuk sawit yang bagus?": "What's a good way to fertilize palm oil trees?",
            "Apa hama yang sering menyerang sawit?": "What pests commonly attack palm oil plants?",
            "Bagaimana panen sawit yang efektif?": "How to harvest palm oil effectively?"
        },
        ("en", "id"): {
            "Why are my palm leaves yellow?": "Kenapa daun sawit saya kuning?",
            "What's a good way to fertilize palm oil trees?": "Bagaimana cara pupuk sawit yang bagus?",
            "What pests commonly attack palm oil plants?": "Apa hama yang sering menyerang sawit?",
            "How to harvest palm oil effectively?": "Bagaimana panen sawit yang efektif?"
        }
    }
    
    # Get the appropriate translation dictionary
    trans_dict = translations.get((from_lang, to_lang), {})
    
    # Try exact match first for common phrases
    if text.strip() in trans_dict:
        return trans_dict[text.strip()]
    
    # Use AI translation for longer or complex text
    try:
        target_language = "English" if to_lang == "en" else "Indonesian"
        source_language = "Indonesian" if from_lang == "id" else "English"
        
        translation_prompt = f"""Translate the following text from {source_language} to {target_language}. 
        This is about palm oil farming. Provide only the translation, no explanations:

        Text to translate: {text}
        
        Translation:"""
        
        response = client.chat_completion(
            model="meta-llama/Llama-3.2-3B-Instruct",
            messages=[{"role": "user", "content": translation_prompt}],
            temperature=0.3,
            max_tokens=300,
            top_p=0.9
        )
        
        translated_text = response.choices[0].message.content.strip()
        
        # Clean up the response - remove any extra text
        if "Translation:" in translated_text:
            translated_text = translated_text.split("Translation:")[-1].strip()
        if translated_text.startswith('"') and translated_text.endswith('"'):
            translated_text = translated_text[1:-1]
        
        return translated_text
        
    except Exception as e:
        # Fallback to keyword mapping if AI translation fails
        translated_text = text
        sorted_translations = sorted(trans_dict.items(), key=lambda x: len(x[0]), reverse=True)
        
        for original, translation in sorted_translations:
            if original.lower() in translated_text.lower():
                import re
                pattern = re.compile(re.escape(original), re.IGNORECASE)
                translated_text = pattern.sub(translation, translated_text)
        
        return translated_text

# --- Language Selection ---
if "language" not in st.session_state:
    st.session_state.language = "id"  # Default: Bahasa Indonesia

# --- Text Translations ---
TEXT = {
    "id": {
        "title": "🌴 PalmPal – Teman Berkebun Kamu",
        "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
        "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
        "send_button": "📨 Kirim",
        "examples": "Contoh Pertanyaan:",
        "advanced": "⚙️ Pengaturan Lanjutan",
        "clear_chat": "🧹 Bersihkan Riwayat Chat",
        "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
        "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
        "thinking": "PalmPal sedang berpikir...",
        "user": "Kamu",
        "bot": "PalmPal",
        "error": "Maaf, PalmPal mengalami gangguan.",
        "lang_toggle": "🌐 Switch to English"
    },
    "en": {
        "title": "🌴 PalmPal – You Farming Friend",
        "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
        "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
        "send_button": "📨 Send",
        "examples": "Example Questions:",
        "advanced": "⚙️ Advanced Settings",
        "clear_chat": "🧹 Clear Chat History",
        "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
        "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
        "thinking": "PalmPal is thinking...",
        "user": "You",
        "bot": "PalmPal",
        "error": "Sorry, PalmPal encountered an error.",
        "lang_toggle": "🌐 Ganti ke Bahasa Indonesia"
    }
}
lang = st.session_state.language
t = TEXT[lang]

# --- Restored Language Toggle Button with AI Translation Feature ---
st.markdown('<div class="lang-toggle">', unsafe_allow_html=True)
lang_toggle = st.button(t["lang_toggle"])
if lang_toggle:
    # Store the old language before switching
    old_lang = st.session_state.language
    new_lang = "en" if st.session_state.language == "id" else "id"
    
    # Translate existing messages when language is switched
    if 'messages' in st.session_state and st.session_state.messages:
        with st.spinner("Translating messages..." if new_lang == "en" else "Menerjemahkan pesan..."):
            translated_messages = []
            for message in st.session_state.messages:
                original_content = message["content"]
                translated_content = translate_content(original_content, old_lang, new_lang)
                translated_messages.append({
                    "role": message["role"],
                    "content": translated_content
                })
            
            st.session_state.messages = translated_messages
    
    # Switch the language
    st.session_state.language = new_lang
    st.rerun()
st.markdown('</div>', unsafe_allow_html=True)

# --- Only use one st.image with smaller dimensions ---
st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=120)
st.markdown(f"## {t['title']}")
st.markdown(t['subtitle'])

# --- Session State Initialization ---
if 'messages' not in st.session_state:
    st.session_state.messages = []

# --- Generate Bot Response ---
def generate_response(user_input):
    messages = []
    for msg in st.session_state.messages[-10:]:
        role = "user" if msg["role"] == "user" else "assistant"
        messages.append({"role": role, "content": msg["content"]})
    messages.append({"role": "user", "content": user_input})

    with st.spinner(t['thinking']):
        try:
            response = client.chat_completion(
                model="meta-llama/Llama-3.2-3B-Instruct",
                messages=messages,
                temperature=0.7,
                max_tokens=500,
                top_p=0.9
            )
            return response.choices[0].message.content
        except Exception as e:
            st.error(f"{t['error']}: {str(e)}")
            return t['error']

# --- Display Chat Bubbles ---
for msg in st.session_state.messages[-10:]:
    if msg["role"] == "user":
        st.markdown(f"""
        <div class="chat-container user">
            <div class="chat-bubble user-bubble">{msg['content']}</div>
            <div class="chat-icon user-icon"></div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div class="chat-container bot">
            <div class="chat-icon bot-icon"></div>
            <div class="chat-bubble bot-bubble">{msg['content']}</div>
        </div>
        """, unsafe_allow_html=True)

# --- Example Prompts RIGHT ABOVE INPUT ---
st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

example_prompts = {
    "id": [
        "Kenapa daun sawit saya kuning?",
        "Bagaimana cara pupuk sawit yang bagus?",
        "Apa hama yang sering menyerang sawit?",
        "Bagaimana panen sawit yang efektif?"
    ],
    "en": [
        "Why are my palm leaves yellow?",
        "What's a good way to fertilize palm oil trees?",
        "What pests commonly attack palm oil plants?",
        "How to harvest palm oil effectively?"
    ]
}

# Custom HTML grid for examples with equal spacing
st.markdown('<div class="example-grid">', unsafe_allow_html=True)
for i, prompt in enumerate(example_prompts[lang]):
    if st.button(prompt, key=f"example_{i}"):
        st.session_state.messages.append({"role": "user", "content": prompt})
        bot_response = generate_response(prompt)
        st.session_state.messages.append({"role": "assistant", "content": bot_response})
        st.rerun()
st.markdown('</div>', unsafe_allow_html=True)

# --- User Input ---
def process_user_input():
    if st.session_state.user_input:
        user_message = st.session_state.user_input
        st.session_state.messages.append({"role": "user", "content": user_message})
        bot_response = generate_response(user_message)
        st.session_state.messages.append({"role": "assistant", "content": bot_response})
        st.session_state.user_input = ""  # Clear input field

# Input area with better alignment
col1, col2 = st.columns([4, 1])
with col1:
    st.text_input(t["input_placeholder"], key="user_input", on_change=process_user_input)
with col2:
    st.markdown('<div class="send-button">', unsafe_allow_html=True)
    st.button(t["send_button"], on_click=process_user_input)
    st.markdown('</div>', unsafe_allow_html=True)

# --- Optional Advanced Section ---
with st.expander(t["advanced"]):
    if st.button(t["clear_chat"]):
        st.session_state.messages = []
        st.rerun()
    st.markdown(t["model_info"])
























# import streamlit as st
# import requests
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Reduce space but not as extreme as before */
#     .block-container {
#         padding-top: 1rem !important;
#         padding-bottom: 0rem !important;
#         margin-top: -2rem !important;
#     }
    
#     /* Further reduce space in the main area */
#     .main .block-container {
#         padding: 1rem !important; 
#         max-width: 100% !important;
#     }
    
#     /* Reduce spaces and gaps - but leave some breathing room */
#     .css-1y4p8pa {
#         margin-top: -1rem !important;
#         padding-top: 0.5rem !important;
#         padding-bottom: 0rem !important;
#     }
    
#     /* Reduce space above the chat messages */
#     .css-1544g2n {
#         padding-top: 0.5rem !important;
#         margin-top: 0 !important;
#     }
    
#     /* Reduce any extra margins around elements */
#     .element-container, .stMarkdown, .stButton, .stImage {
#         margin-top: 0.2rem !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#         padding-bottom: 0 !important;
#     }
    
#     /* Reduce space around image */
#     img {
#         margin-bottom: 0.5rem !important;
#     }
    
#     /* Keep more breathing room in title and subtitle */
#     h2 {
#         margin-top: 0 !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#     }
    
#     p {
#         margin-bottom: 0.5rem !important;
#         padding: 0 !important;
#         line-height: 1.2 !important;
#     }
    
#     /* Chat bubbles with icons - slightly more room */
#     .chat-container {
#         display: flex;
#         margin-bottom: 8px !important;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 10px 14px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Mode selection buttons */
#     .mode-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 15px;
#     }
    
#     .mode-grid button {
#         width: 100%;
#         padding: 10px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.9em !important;
#         font-weight: bold !important;
#     }
    
#     /* Restore SawitPRO brand colors for example prompts */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 8px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.95em !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 5px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Language toggle button styling */
#     .lang-toggle {
#         display: inline-block;
#         margin-bottom: 8px;
#     }
    
#     .lang-toggle button {
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         border: none !important;
#         font-size: 0.9em !important;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Add moderate padding to gently push things up */
#     .filler {
#         height: 1px;
#         margin-top: -150px !important;
#     }
    
#     /* Hide footer but keep header */
#     footer {
#         display: none !important;
#     }
    
#     /* Make textfields look better */
#     .stTextInput > div {
#         margin-top: 0 !important;
#         margin-bottom: 8px !important;
#     }
    
#     /* Custom styling for buttons */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Current mode indicator */
#     .current-mode {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         padding: 5px 10px;
#         border-radius: 15px;
#         font-size: 0.8em;
#         margin-bottom: 10px;
#         text-align: center;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Add a filler div to push things up but not too aggressively ---
# st.markdown('<div class="filler"></div>', unsafe_allow_html=True)

# # --- API Functions ---
# def get_user_recommendations(user_id):
#     """Fetch user recommendations from API"""
#     try:
#         url = f"https://icp-t02-grp4-api.onrender.com/api/v1/ecommerce/recommendations/user/{user_id}"
#         response = requests.get(url, timeout=10)
        
#         if response.status_code == 200:
#             data = response.json()
#             return True, data
#         else:
#             return False, f"Error: {response.status_code} - Invalid user ID or API error"
#     except requests.RequestException as e:
#         return False, f"Network error: {str(e)}"

# def get_product_recommendations(product_id):
#     """Fetch product recommendations from API"""
#     try:
#         url = f"https://icp-t02-grp4-api.onrender.com/api/api/v1/ecommerce/recommendations/products/{product_id}"
#         response = requests.get(url, timeout=10)
        
#         if response.status_code == 200:
#             data = response.json()
#             return True, data
#         else:
#             return False, f"Error: {response.status_code} - Invalid product ID or API error"
#     except requests.RequestException as e:
#         return False, f"Network error: {str(e)}"

# def format_recommendations(data, recommendation_type):
#     """Format recommendation data for display"""
#     if recommendation_type == "user":
#         if "recommendations" in data and data["recommendations"]:
#             formatted = "🛍️ **Recommended Products for You:**\n\n"
#             for i, item in enumerate(data["recommendations"][:5], 1):  # Show top 5
#                 formatted += f"{i}. **{item.get('name', 'Unknown Product')}**\n"
#                 formatted += f"   Price: ${item.get('price', 'N/A')}\n"
#                 formatted += f"   Category: {item.get('category', 'N/A')}\n"
#                 formatted += f"   Rating: {item.get('rating', 'N/A')} ⭐\n\n"
#             return formatted
#         else:
#             return "No recommendations found for this user."
    
#     elif recommendation_type == "product":
#         if "similar_products" in data and data["similar_products"]:
#             formatted = "🔍 **Similar Products:**\n\n"
#             for i, item in enumerate(data["similar_products"][:5], 1):  # Show top 5
#                 formatted += f"{i}. **{item.get('name', 'Unknown Product')}**\n"
#                 formatted += f"   Price: ${item.get('price', 'N/A')}\n"
#                 formatted += f"   Category: {item.get('category', 'N/A')}\n"
#                 formatted += f"   Rating: {item.get('rating', 'N/A')} ⭐\n\n"
#             return formatted
#         else:
#             return "No similar products found."
    
#     return "Unable to format recommendations."

# # --- Translation function for existing messages ---
# def translate_content(text, from_lang, to_lang):
#     """Translation function that uses both keyword mapping and AI translation."""
#     if from_lang == to_lang:
#         return text
    
#     # Define translation mappings for quick common phrases
#     translations = {
#         ("id", "en"): {
#             "Kenapa daun sawit saya kuning?": "Why are my palm leaves yellow?",
#             "Bagaimana cara pupuk sawit yang bagus?": "What's a good way to fertilize palm oil trees?",
#             "Apa hama yang sering menyerang sawit?": "What pests commonly attack palm oil plants?",
#             "Bagaimana panen sawit yang efektif?": "How to harvest palm oil effectively?"
#         },
#         ("en", "id"): {
#             "Why are my palm leaves yellow?": "Kenapa daun sawit saya kuning?",
#             "What's a good way to fertilize palm oil trees?": "Bagaimana cara pupuk sawit yang bagus?",
#             "What pests commonly attack palm oil plants?": "Apa hama yang sering menyerang sawit?",
#             "How to harvest palm oil effectively?": "Bagaimana panen sawit yang efektif?"
#         }
#     }
    
#     # Get the appropriate translation dictionary
#     trans_dict = translations.get((from_lang, to_lang), {})
    
#     # Try exact match first for common phrases
#     if text.strip() in trans_dict:
#         return trans_dict[text.strip()]
    
#     # Use AI translation for longer or complex text
#     try:
#         target_language = "English" if to_lang == "en" else "Indonesian"
#         source_language = "Indonesian" if from_lang == "id" else "English"
        
#         translation_prompt = f"""Translate the following text from {source_language} to {target_language}. 
#         This is about palm oil farming. Provide only the translation, no explanations:

#         Text to translate: {text}
        
#         Translation:"""
        
#         response = client.chat_completion(
#             model="meta-llama/Llama-3.2-3B-Instruct",
#             messages=[{"role": "user", "content": translation_prompt}],
#             temperature=0.3,
#             max_tokens=300,
#             top_p=0.9
#         )
        
#         translated_text = response.choices[0].message.content.strip()
        
#         # Clean up the response - remove any extra text
#         if "Translation:" in translated_text:
#             translated_text = translated_text.split("Translation:")[-1].strip()
#         if translated_text.startswith('"') and translated_text.endswith('"'):
#             translated_text = translated_text[1:-1]
        
#         return translated_text
        
#     except Exception as e:
#         # Fallback to keyword mapping if AI translation fails
#         translated_text = text
#         sorted_translations = sorted(trans_dict.items(), key=lambda x: len(x[0]), reverse=True)
        
#         for original, translation in sorted_translations:
#             if original.lower() in translated_text.lower():
#                 import re
#                 pattern = re.compile(re.escape(original), re.IGNORECASE)
#                 translated_text = pattern.sub(translation, translated_text)
        
#         return translated_text

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# if 'current_mode' not in st.session_state:
#     st.session_state.current_mode = "general"  # Default mode

# if 'waiting_for_input' not in st.session_state:
#     st.session_state.waiting_for_input = None  # None, "user_id", "product_id"

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan.",
#         "lang_toggle": "🌐 Switch to English",
#         "mode_user": "👤 Rekomendasi Pengguna",
#         "mode_product": "📦 Rekomendasi Produk", 
#         "mode_general": "💬 Chat Umum",
#         "current_mode_text": "Mode saat ini:",
#         "enter_user_id": "Masukkan User ID Anda:",
#         "enter_product_id": "Masukkan Product ID:",
#         "mode_selection": "Pilih mode yang ingin Anda gunakan:"
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error.",
#         "lang_toggle": "🌐 Ganti ke Bahasa Indonesia",
#         "mode_user": "👤 User Recommendations",
#         "mode_product": "📦 Product Recommendations",
#         "mode_general": "💬 General Chat",
#         "current_mode_text": "Current mode:",
#         "enter_user_id": "Enter your User ID:",
#         "enter_product_id": "Enter Product ID:",
#         "mode_selection": "Choose the mode you want to use:"
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Language Toggle Button with AI Translation Feature ---
# st.markdown('<div class="lang-toggle">', unsafe_allow_html=True)
# lang_toggle = st.button(t["lang_toggle"])
# if lang_toggle:
#     # Store the old language before switching
#     old_lang = st.session_state.language
#     new_lang = "en" if st.session_state.language == "id" else "id"
    
#     # Translate existing messages when language is switched
#     if 'messages' in st.session_state and st.session_state.messages:
#         with st.spinner("Translating messages..." if new_lang == "en" else "Menerjemahkan pesan..."):
#             translated_messages = []
#             for message in st.session_state.messages:
#                 original_content = message["content"]
#                 translated_content = translate_content(original_content, old_lang, new_lang)
#                 translated_messages.append({
#                     "role": message["role"],
#                     "content": translated_content
#                 })
            
#             st.session_state.messages = translated_messages
    
#     # Switch the language
#     st.session_state.language = new_lang
#     st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=120)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Mode Selection Buttons ---
# st.markdown(f"**{t['mode_selection']}**")
# st.markdown('<div class="mode-grid">', unsafe_allow_html=True)

# col1, col2, col3 = st.columns(3)
# with col1:
#     if st.button(t["mode_user"], key="mode_user_btn"):
#         st.session_state.current_mode = "user"
#         st.session_state.waiting_for_input = "user_id"
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": t["enter_user_id"]
#         })
#         st.rerun()

# with col2:
#     if st.button(t["mode_product"], key="mode_product_btn"):
#         st.session_state.current_mode = "product"
#         st.session_state.waiting_for_input = "product_id"
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": t["enter_product_id"]
#         })
#         st.rerun()

# with col3:
#     if st.button(t["mode_general"], key="mode_general_btn"):
#         st.session_state.current_mode = "general"
#         st.session_state.waiting_for_input = None
#         if lang == "id":
#             welcome_msg = "Selamat datang di mode chat umum! Tanyakan apa saja tentang pertanian sawit."
#         else:
#             welcome_msg = "Welcome to general chat mode! Ask anything about palm oil farming."
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": welcome_msg
#         })
#         st.rerun()

# st.markdown('</div>', unsafe_allow_html=True)

# # --- Current Mode Indicator ---
# mode_names = {
#     "user": t["mode_user"],
#     "product": t["mode_product"], 
#     "general": t["mode_general"]
# }
# st.markdown(f'<div class="current-mode">{t["current_mode_text"]} {mode_names[st.session_state.current_mode]}</div>', unsafe_allow_html=True)

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Process API Requests ---
# def process_api_request(user_input):
#     """Process user input based on current waiting state"""
#     if st.session_state.waiting_for_input == "user_id":
#         # User provided user ID
#         with st.spinner("Fetching user recommendations..." if lang == "en" else "Mengambil rekomendasi pengguna..."):
#             success, data = get_user_recommendations(user_input.strip())
            
#             if success:
#                 formatted_response = format_recommendations(data, "user")
#                 # Add option to return to mode selection
#                 if lang == "id":
#                     formatted_response += "\n\n✨ Rekomendasi selesai! Silakan pilih mode lagi atau lanjutkan chat umum."
#                 else:
#                     formatted_response += "\n\n✨ Recommendations complete! Please select a mode again or continue with general chat."
#             else:
#                 formatted_response = data  # Error message
                
#         st.session_state.waiting_for_input = None
#         st.session_state.current_mode = "general"
#         return formatted_response
        
#     elif st.session_state.waiting_for_input == "product_id":
#         # User provided product ID  
#         with st.spinner("Fetching product recommendations..." if lang == "en" else "Mengambil rekomendasi produk..."):
#             success, data = get_product_recommendations(user_input.strip())
            
#             if success:
#                 formatted_response = format_recommendations(data, "product")
#                 # Add option to return to mode selection
#                 if lang == "id":
#                     formatted_response += "\n\n✨ Rekomendasi selesai! Silakan pilih mode lagi atau lanjutkan chat umum."
#                 else:
#                     formatted_response += "\n\n✨ Recommendations complete! Please select a mode again or continue with general chat."
#             else:
#                 formatted_response = data  # Error message
                
#         st.session_state.waiting_for_input = None
#         st.session_state.current_mode = "general"
#         return formatted_response
    
#     else:
#         # General chat mode
#         return generate_response(user_input)

# # --- Display Chat Bubbles ---
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # --- Example Prompts (only show for general mode) ---
# if st.session_state.current_mode == "general" and st.session_state.waiting_for_input is None:
#     st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

#     example_prompts = {
#         "id": [
#             "Kenapa daun sawit saya kuning?",
#             "Bagaimana cara pupuk sawit yang bagus?",
#             "Apa hama yang sering menyerang sawit?",
#             "Bagaimana panen sawit yang efektif?"
#         ],
#         "en": [
#             "Why are my palm leaves yellow?",
#             "What's a good way to fertilize palm oil trees?",
#             "What pests commonly attack palm oil plants?",
#             "How to harvest palm oil effectively?"
#         ]
#     }

#     # Custom HTML grid for examples with equal spacing
#     st.markdown('<div class="example-grid">', unsafe_allow_html=True)
#     for i, prompt in enumerate(example_prompts[lang]):
#         if st.button(prompt, key=f"example_{i}"):
#             st.session_state.messages.append({"role": "user", "content": prompt})
#             bot_response = generate_response(prompt)
#             st.session_state.messages.append({"role": "assistant", "content": bot_response})
#             st.rerun()
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
        
#         # Process based on current mode and waiting state
#         bot_response = process_api_request(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     placeholder_text = t["input_placeholder"]
#     if st.session_state.waiting_for_input == "user_id":
#         placeholder_text = t["enter_user_id"]
#     elif st.session_state.waiting_for_input == "product_id":
#         placeholder_text = t["enter_product_id"]
        
#     st.text_input(placeholder_text, key="user_input", on_change=process_user_input)
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.session_state.current_mode = "general"
#         st.session_state.waiting_for_input = None
#         st.rerun()
#     st.markdown(t["model_info"])


























# import streamlit as st
# import requests
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Reduce space but not as extreme as before */
#     .block-container {
#         padding-top: 1rem !important;
#         padding-bottom: 0rem !important;
#         margin-top: -2rem !important;
#     }
    
#     /* Further reduce space in the main area */
#     .main .block-container {
#         padding: 1rem !important; 
#         max-width: 100% !important;
#     }
    
#     /* Reduce spaces and gaps - but leave some breathing room */
#     .css-1y4p8pa {
#         margin-top: -1rem !important;
#         padding-top: 0.5rem !important;
#         padding-bottom: 0rem !important;
#     }
    
#     /* Reduce space above the chat messages */
#     .css-1544g2n {
#         padding-top: 0.5rem !important;
#         margin-top: 0 !important;
#     }
    
#     /* Reduce any extra margins around elements */
#     .element-container, .stMarkdown, .stButton, .stImage {
#         margin-top: 0.2rem !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#         padding-bottom: 0 !important;
#     }
    
#     /* Reduce space around image */
#     img {
#         margin-bottom: 0.5rem !important;
#     }
    
#     /* Keep more breathing room in title and subtitle */
#     h2 {
#         margin-top: 0 !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#     }
    
#     p {
#         margin-bottom: 0.5rem !important;
#         padding: 0 !important;
#         line-height: 1.2 !important;
#     }
    
#     /* Chat bubbles with icons - slightly more room */
#     .chat-container {
#         display: flex;
#         margin-bottom: 8px !important;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 10px 14px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Mode selection buttons */
#     .mode-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 15px;
#     }
    
#     .mode-grid button {
#         width: 100%;
#         padding: 10px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.9em !important;
#         font-weight: bold !important;
#     }
    
#     /* Restore SawitPRO brand colors for example prompts */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 8px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.95em !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 5px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Language toggle button styling */
#     .lang-toggle {
#         display: inline-block;
#         margin-bottom: 8px;
#     }
    
#     .lang-toggle button {
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         border: none !important;
#         font-size: 0.9em !important;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Add moderate padding to gently push things up */
#     .filler {
#         height: 1px;
#         margin-top: -150px !important;
#     }
    
#     /* Hide footer but keep header */
#     footer {
#         display: none !important;
#     }
    
#     /* Make textfields look better */
#     .stTextInput > div {
#         margin-top: 0 !important;
#         margin-bottom: 8px !important;
#     }
    
#     /* Custom styling for buttons */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Current mode indicator */
#     .current-mode {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         padding: 5px 10px;
#         border-radius: 15px;
#         font-size: 0.8em;
#         margin-bottom: 10px;
#         text-align: center;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Add a filler div to push things up but not too aggressively ---
# st.markdown('<div class="filler"></div>', unsafe_allow_html=True)

# # --- API Functions ---
# def get_user_recommendations(user_id):
#     """Fetch user recommendations from API"""
#     try:
#         url = f"https://icp-t02-grp4-api.onrender.com/api/v1/ecommerce/recommendations/user/{user_id}"
#         response = requests.get(url, timeout=10)
        
#         if response.status_code == 200:
#             data = response.json()
#             return True, data
#         else:
#             return False, f"Error: {response.status_code} - Invalid user ID or API error"
#     except requests.RequestException as e:
#         return False, f"Network error: {str(e)}"

# def get_product_recommendations(product_id):
#     """Fetch product recommendations from API"""
#     try:
#         url = f"https://icp-t02-grp4-api.onrender.com/api/api/v1/ecommerce/recommendations/products/{product_id}"
#         response = requests.get(url, timeout=10)
        
#         if response.status_code == 200:
#             data = response.json()
#             return True, data
#         else:
#             return False, f"Error: {response.status_code} - Invalid product ID or API error"
#     except requests.RequestException as e:
#         return False, f"Network error: {str(e)}"

# def format_recommendations(data, recommendation_type):
#     """Format recommendation data for display"""
#     if recommendation_type == "user":
#         if "recommendations" in data and data["recommendations"]:
#             formatted = "🛍️ **Recommended Products for You:**\n\n"
#             for i, item in enumerate(data["recommendations"][:5], 1):  # Show top 5
#                 # Use the correct field names from API response
#                 product_name = item.get('product_name_en', 'Unknown Product')
#                 product_id = item.get('product_id', 'N/A')
#                 score = item.get('score', 'N/A')
#                 description = item.get('product_description_en', '')
                
#                 formatted += f"{i}. **{product_name}**\n"
#                 formatted += f"   Product ID: {product_id}\n"
#                 formatted += f"   Recommendation Score: {score:.2f if isinstance(score, (int, float)) else score}\n"
#                 if description:
#                     # Truncate long descriptions
#                     short_desc = description[:100] + "..." if len(description) > 100 else description
#                     formatted += f"   Description: {short_desc}\n"
#                 formatted += "\n"
#             return formatted
#         else:
#             return "No recommendations found for this user."
    
#     elif recommendation_type == "product":
#         if "similar_products" in data and data["similar_products"]:
#             formatted = "🔍 **Similar Products:**\n\n"
#             for i, item in enumerate(data["similar_products"][:5], 1):  # Show top 5
#                 # Use the correct field names from API response
#                 product_name = item.get('product_name_en', item.get('name', 'Unknown Product'))
#                 product_id = item.get('product_id', item.get('id', 'N/A'))
#                 score = item.get('score', item.get('similarity_score', 'N/A'))
#                 description = item.get('product_description_en', item.get('description', ''))
                
#                 formatted += f"{i}. **{product_name}**\n"
#                 formatted += f"   Product ID: {product_id}\n"
#                 formatted += f"   Similarity Score: {score:.2f if isinstance(score, (int, float)) else score}\n"
#                 if description:
#                     # Truncate long descriptions
#                     short_desc = description[:100] + "..." if len(description) > 100 else description
#                     formatted += f"   Description: {short_desc}\n"
#                 formatted += "\n"
#             return formatted
#         else:
#             return "No similar products found."
    
#     return "Unable to format recommendations."

# # --- Translation function for existing messages ---
# def translate_content(text, from_lang, to_lang):
#     """Translation function that uses both keyword mapping and AI translation."""
#     if from_lang == to_lang:
#         return text
    
#     # Define translation mappings for quick common phrases
#     translations = {
#         ("id", "en"): {
#             "Kenapa daun sawit saya kuning?": "Why are my palm leaves yellow?",
#             "Bagaimana cara pupuk sawit yang bagus?": "What's a good way to fertilize palm oil trees?",
#             "Apa hama yang sering menyerang sawit?": "What pests commonly attack palm oil plants?",
#             "Bagaimana panen sawit yang efektif?": "How to harvest palm oil effectively?"
#         },
#         ("en", "id"): {
#             "Why are my palm leaves yellow?": "Kenapa daun sawit saya kuning?",
#             "What's a good way to fertilize palm oil trees?": "Bagaimana cara pupuk sawit yang bagus?",
#             "What pests commonly attack palm oil plants?": "Apa hama yang sering menyerang sawit?",
#             "How to harvest palm oil effectively?": "Bagaimana panen sawit yang efektif?"
#         }
#     }
    
#     # Get the appropriate translation dictionary
#     trans_dict = translations.get((from_lang, to_lang), {})
    
#     # Try exact match first for common phrases
#     if text.strip() in trans_dict:
#         return trans_dict[text.strip()]
    
#     # Use AI translation for longer or complex text
#     try:
#         target_language = "English" if to_lang == "en" else "Indonesian"
#         source_language = "Indonesian" if from_lang == "id" else "English"
        
#         translation_prompt = f"""Translate the following text from {source_language} to {target_language}. 
#         This is about palm oil farming. Provide only the translation, no explanations:

#         Text to translate: {text}
        
#         Translation:"""
        
#         response = client.chat_completion(
#             model="meta-llama/Llama-3.2-3B-Instruct",
#             messages=[{"role": "user", "content": translation_prompt}],
#             temperature=0.3,
#             max_tokens=300,
#             top_p=0.9
#         )
        
#         translated_text = response.choices[0].message.content.strip()
        
#         # Clean up the response - remove any extra text
#         if "Translation:" in translated_text:
#             translated_text = translated_text.split("Translation:")[-1].strip()
#         if translated_text.startswith('"') and translated_text.endswith('"'):
#             translated_text = translated_text[1:-1]
        
#         return translated_text
        
#     except Exception as e:
#         # Fallback to keyword mapping if AI translation fails
#         translated_text = text
#         sorted_translations = sorted(trans_dict.items(), key=lambda x: len(x[0]), reverse=True)
        
#         for original, translation in sorted_translations:
#             if original.lower() in translated_text.lower():
#                 import re
#                 pattern = re.compile(re.escape(original), re.IGNORECASE)
#                 translated_text = pattern.sub(translation, translated_text)
        
#         return translated_text

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# if 'current_mode' not in st.session_state:
#     st.session_state.current_mode = "general"  # Default mode

# if 'waiting_for_input' not in st.session_state:
#     st.session_state.waiting_for_input = None  # None, "user_id", "product_id"

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan.",
#         "lang_toggle": "🌐 Switch to English",
#         "mode_user": "👤 Rekomendasi Pengguna",
#         "mode_product": "📦 Rekomendasi Produk", 
#         "mode_general": "💬 Chat Umum",
#         "current_mode_text": "Mode saat ini:",
#         "enter_user_id": "Masukkan User ID Anda:",
#         "enter_product_id": "Masukkan Product ID:",
#         "mode_selection": "Pilih mode yang ingin Anda gunakan:"
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error.",
#         "lang_toggle": "🌐 Ganti ke Bahasa Indonesia",
#         "mode_user": "👤 User Recommendations",
#         "mode_product": "📦 Product Recommendations",
#         "mode_general": "💬 General Chat",
#         "current_mode_text": "Current mode:",
#         "enter_user_id": "Enter your User ID:",
#         "enter_product_id": "Enter Product ID:",
#         "mode_selection": "Choose the mode you want to use:"
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Language Toggle Button with AI Translation Feature ---
# st.markdown('<div class="lang-toggle">', unsafe_allow_html=True)
# lang_toggle = st.button(t["lang_toggle"])
# if lang_toggle:
#     # Store the old language before switching
#     old_lang = st.session_state.language
#     new_lang = "en" if st.session_state.language == "id" else "id"
    
#     # Translate existing messages when language is switched
#     if 'messages' in st.session_state and st.session_state.messages:
#         with st.spinner("Translating messages..." if new_lang == "en" else "Menerjemahkan pesan..."):
#             translated_messages = []
#             for message in st.session_state.messages:
#                 original_content = message["content"]
#                 translated_content = translate_content(original_content, old_lang, new_lang)
#                 translated_messages.append({
#                     "role": message["role"],
#                     "content": translated_content
#                 })
            
#             st.session_state.messages = translated_messages
    
#     # Switch the language
#     st.session_state.language = new_lang
#     st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=120)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Mode Selection Buttons ---
# st.markdown(f"**{t['mode_selection']}**")
# st.markdown('<div class="mode-grid">', unsafe_allow_html=True)

# col1, col2, col3 = st.columns(3)
# with col1:
#     if st.button(t["mode_user"], key="mode_user_btn"):
#         st.session_state.current_mode = "user"
#         st.session_state.waiting_for_input = "user_id"
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": t["enter_user_id"]
#         })
#         st.rerun()

# with col2:
#     if st.button(t["mode_product"], key="mode_product_btn"):
#         st.session_state.current_mode = "product"
#         st.session_state.waiting_for_input = "product_id"
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": t["enter_product_id"]
#         })
#         st.rerun()

# with col3:
#     if st.button(t["mode_general"], key="mode_general_btn"):
#         st.session_state.current_mode = "general"
#         st.session_state.waiting_for_input = None
#         if lang == "id":
#             welcome_msg = "Selamat datang di mode chat umum! Tanyakan apa saja tentang pertanian sawit."
#         else:
#             welcome_msg = "Welcome to general chat mode! Ask anything about palm oil farming."
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": welcome_msg
#         })
#         st.rerun()

# st.markdown('</div>', unsafe_allow_html=True)

# # --- Current Mode Indicator ---
# mode_names = {
#     "user": t["mode_user"],
#     "product": t["mode_product"], 
#     "general": t["mode_general"]
# }
# st.markdown(f'<div class="current-mode">{t["current_mode_text"]} {mode_names[st.session_state.current_mode]}</div>', unsafe_allow_html=True)

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Process API Requests ---
# def process_api_request(user_input):
#     """Process user input based on current waiting state"""
#     if st.session_state.waiting_for_input == "user_id":
#         # User provided user ID
#         with st.spinner("Fetching user recommendations..." if lang == "en" else "Mengambil rekomendasi pengguna..."):
#             success, data = get_user_recommendations(user_input.strip())
            
#             if success:
#                 formatted_response = format_recommendations(data, "user")
#                 # Add option to return to mode selection
#                 if lang == "id":
#                     formatted_response += "\n\n✨ Rekomendasi selesai! Silakan pilih mode lagi atau lanjutkan chat umum."
#                 else:
#                     formatted_response += "\n\n✨ Recommendations complete! Please select a mode again or continue with general chat."
#             else:
#                 formatted_response = data  # Error message
                
#         st.session_state.waiting_for_input = None
#         st.session_state.current_mode = "general"
#         return formatted_response
        
#     elif st.session_state.waiting_for_input == "product_id":
#         # User provided product ID  
#         with st.spinner("Fetching product recommendations..." if lang == "en" else "Mengambil rekomendasi produk..."):
#             success, data = get_product_recommendations(user_input.strip())
            
#             if success:
#                 formatted_response = format_recommendations(data, "product")
#                 # Add option to return to mode selection
#                 if lang == "id":
#                     formatted_response += "\n\n✨ Rekomendasi selesai! Silakan pilih mode lagi atau lanjutkan chat umum."
#                 else:
#                     formatted_response += "\n\n✨ Recommendations complete! Please select a mode again or continue with general chat."
#             else:
#                 formatted_response = data  # Error message
                
#         st.session_state.waiting_for_input = None
#         st.session_state.current_mode = "general"
#         return formatted_response
    
#     else:
#         # General chat mode
#         return generate_response(user_input)

# # --- Display Chat Bubbles ---
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # --- Example Prompts (only show for general mode) ---
# if st.session_state.current_mode == "general" and st.session_state.waiting_for_input is None:
#     st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

#     example_prompts = {
#         "id": [
#             "Kenapa daun sawit saya kuning?",
#             "Bagaimana cara pupuk sawit yang bagus?",
#             "Apa hama yang sering menyerang sawit?",
#             "Bagaimana panen sawit yang efektif?"
#         ],
#         "en": [
#             "Why are my palm leaves yellow?",
#             "What's a good way to fertilize palm oil trees?",
#             "What pests commonly attack palm oil plants?",
#             "How to harvest palm oil effectively?"
#         ]
#     }

#     # Custom HTML grid for examples with equal spacing
#     st.markdown('<div class="example-grid">', unsafe_allow_html=True)
#     for i, prompt in enumerate(example_prompts[lang]):
#         if st.button(prompt, key=f"example_{i}"):
#             st.session_state.messages.append({"role": "user", "content": prompt})
#             bot_response = generate_response(prompt)
#             st.session_state.messages.append({"role": "assistant", "content": bot_response})
#             st.rerun()
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
        
#         # Process based on current mode and waiting state
#         bot_response = process_api_request(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     placeholder_text = t["input_placeholder"]
#     if st.session_state.waiting_for_input == "user_id":
#         placeholder_text = t["enter_user_id"]
#     elif st.session_state.waiting_for_input == "product_id":
#         placeholder_text = t["enter_product_id"]
        
#     # Remove on_change to prevent double sending
#     st.text_input(placeholder_text, key="user_input")
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.session_state.current_mode = "general"
#         st.session_state.waiting_for_input = None
#         st.rerun()
#     st.markdown(t["model_info"])














# import streamlit as st
# import requests
# from huggingface_hub import InferenceClient

# api_key = st.secrets["HUGGINGFACE_API_KEY"]
# client = InferenceClient(api_key=api_key)

# # --- Page Configuration ---
# st.set_page_config(page_title="PalmPal", page_icon="🌴", layout="centered")

# # --- Custom CSS with SawitPRO colors and icons ---
# st.markdown("""
# <style>
#     /* Main colors */
#     :root {
#         --sawit-green: #576A34;
#         --sawit-yellow: #E9D037;
#         --sawit-dark-green: #3A4A1F;
#     }
    
#     /* Reduce space but not as extreme as before */
#     .block-container {
#         padding-top: 1rem !important;
#         padding-bottom: 0rem !important;
#         margin-top: -2rem !important;
#     }
    
#     /* Further reduce space in the main area */
#     .main .block-container {
#         padding: 1rem !important; 
#         max-width: 100% !important;
#     }
    
#     /* Reduce spaces and gaps - but leave some breathing room */
#     .css-1y4p8pa {
#         margin-top: -1rem !important;
#         padding-top: 0.5rem !important;
#         padding-bottom: 0rem !important;
#     }
    
#     /* Reduce space above the chat messages */
#     .css-1544g2n {
#         padding-top: 0.5rem !important;
#         margin-top: 0 !important;
#     }
    
#     /* Reduce any extra margins around elements */
#     .element-container, .stMarkdown, .stButton, .stImage {
#         margin-top: 0.2rem !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#         padding-bottom: 0 !important;
#     }
    
#     /* Reduce space around image */
#     img {
#         margin-bottom: 0.5rem !important;
#     }
    
#     /* Keep more breathing room in title and subtitle */
#     h2 {
#         margin-top: 0 !important;
#         margin-bottom: 0.2rem !important;
#         padding-top: 0 !important;
#     }
    
#     p {
#         margin-bottom: 0.5rem !important;
#         padding: 0 !important;
#         line-height: 1.2 !important;
#     }
    
#     /* Chat bubbles with icons - slightly more room */
#     .chat-container {
#         display: flex;
#         margin-bottom: 8px !important;
#         max-width: 100%;
#         clear: both;
#     }
    
#     .chat-container.user {
#         justify-content: flex-end;
#     }
    
#     .chat-container.bot {
#         justify-content: flex-start;
#     }
    
#     .chat-icon {
#         width: 36px;
#         height: 36px;
#         border-radius: 50%;
#         background-size: cover;
#         background-position: center;
#         flex-shrink: 0;
#         margin-top: auto;
#     }
    
#     .user-icon {
#         background-image: url('https://rspo.org/wp-content/uploads/AN-OIL-PALM-FARMERS-POSITIVE-IMPACT-ON-THE-ENVIRONMENT_large.jpg');
#         margin-left: 8px;
#     }
    
#     .bot-icon {
#         background-image: url('data:image/png;base64,');
#         margin-right: 8px;
#     }
    
#     .chat-bubble {
#         border-radius: 18px;
#         padding: 10px 14px;
#         max-width: 70%;
#         word-wrap: break-word;
#     }
    
#     .user-bubble {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         border-top-right-radius: 4px;
#     }
    
#     .bot-bubble {
#         background-color: var(--sawit-green);
#         color: white;
#         border-top-left-radius: 4px;
#     }
    
#     /* Mode selection buttons */
#     .mode-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 15px;
#     }
    
#     .mode-grid button {
#         width: 100%;
#         padding: 10px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.9em !important;
#         font-weight: bold !important;
#     }
    
#     /* Restore SawitPRO brand colors for example prompts */
#     .example-grid {
#         display: grid;
#         grid-template-columns: 1fr 1fr;
#         grid-gap: 8px;
#         margin-bottom: 8px;
#     }
    
#     .example-grid button {
#         width: 100%;
#         height: 100%;
#         padding: 8px;
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         font-size: 0.95em !important;
#     }
    
#     /* Example section header */
#     .example-header {
#         margin-bottom: 5px;
#         color: var(--sawit-green);
#         font-weight: bold;
#     }
    
#     /* Language toggle button styling */
#     .lang-toggle {
#         display: inline-block;
#         margin-bottom: 8px;
#     }
    
#     .lang-toggle button {
#         background-color: var(--sawit-green) !important;
#         color: white !important;
#         border: none !important;
#         font-size: 0.9em !important;
#     }
    
#     /* Align send button */
#     .send-button button {
#         margin-top: 24px;
#     }
    
#     /* Header colors */
#     h1, h2, h3, h4, h5, h6 {
#         color: var(--sawit-green);
#     }
    
#     /* Add moderate padding to gently push things up */
#     .filler {
#         height: 1px;
#         margin-top: -150px !important;
#     }
    
#     /* Hide footer but keep header */
#     footer {
#         display: none !important;
#     }
    
#     /* Make textfields look better */
#     .stTextInput > div {
#         margin-top: 0 !important;
#         margin-bottom: 8px !important;
#     }
    
#     /* Custom styling for buttons */
#     div.stButton > button {
#         background-color: var(--sawit-green);
#         color: white;
#         border: none;
#     }
    
#     div.stButton > button:hover {
#         background-color: var(--sawit-dark-green);
#         color: white;
#         border: none;
#     }
    
#     /* Current mode indicator */
#     .current-mode {
#         background-color: var(--sawit-yellow);
#         color: #333;
#         padding: 5px 10px;
#         border-radius: 15px;
#         font-size: 0.8em;
#         margin-bottom: 10px;
#         text-align: center;
#     }
# </style>
# """, unsafe_allow_html=True)

# # --- Add a filler div to push things up but not too aggressively ---
# st.markdown('<div class="filler"></div>', unsafe_allow_html=True)

# # --- API Functions ---
# def get_user_recommendations(user_id):
#     """Fetch user recommendations from API"""
#     try:
#         url = f"https://icp-t02-grp4-api.onrender.com/api/v1/ecommerce/recommendations/user/{user_id}"
#         print(f"Debug: Calling URL: {url}")  # Debug print
#         response = requests.get(url, timeout=15)
        
#         print(f"Debug: Response status: {response.status_code}")  # Debug print
#         print(f"Debug: Response content: {response.text[:500]}")  # Debug print (first 500 chars)
        
#         if response.status_code == 200:
#             data = response.json()
#             return True, data
#         else:
#             return False, f"Error: {response.status_code} - Invalid user ID or API error. Response: {response.text[:200]}"
#     except requests.RequestException as e:
#         return False, f"Network error: {str(e)}"

# def get_product_recommendations(product_id):
#     """Fetch product recommendations from API"""
#     try:
#         # Fixed the URL - removed the duplicate '/api' part
#         url = f"https://icp-t02-grp4-api.onrender.com/api/v1/ecommerce/recommendations/products/{product_id}"
#         print(f"Debug: Calling URL: {url}")  # Debug print
#         response = requests.get(url, timeout=15)
        
#         print(f"Debug: Response status: {response.status_code}")  # Debug print
#         print(f"Debug: Response content: {response.text[:500]}")  # Debug print
        
#         if response.status_code == 200:
#             data = response.json()
#             return True, data
#         else:
#             return False, f"Error: {response.status_code} - Invalid product ID or API error. Response: {response.text[:200]}"
#     except requests.RequestException as e:
#         return False, f"Network error: {str(e)}"

# def format_recommendations(data, recommendation_type):
#     """Format recommendation data for display"""
#     if recommendation_type == "user":
#         if "recommendations" in data and data["recommendations"]:
#             formatted = "🛍️ **Recommended Products for You:**\n\n"
#             for i, item in enumerate(data["recommendations"][:5], 1):  # Show top 5
#                 # Use the correct field names from API response
#                 product_name = item.get('product_name_en', 'Unknown Product')
#                 product_id = item.get('product_id', 'N/A')
#                 score = item.get('score', 'N/A')
#                 description = item.get('product_description_en', '')
                
#                 formatted += f"{i}. **{product_name}**\n"
#                 formatted += f"   Product ID: {product_id}\n"
#                 formatted += f"   Recommendation Score: {score:.2f if isinstance(score, (int, float)) else score}\n"
#                 if description:
#                     # Truncate long descriptions
#                     short_desc = description[:100] + "..." if len(description) > 100 else description
#                     formatted += f"   Description: {short_desc}\n"
#                 formatted += "\n"
#             return formatted
#         else:
#             return "No recommendations found for this user."
    
#     elif recommendation_type == "product":
#         if "similar_products" in data and data["similar_products"]:
#             formatted = "🔍 **Similar Products:**\n\n"
#             for i, item in enumerate(data["similar_products"][:5], 1):  # Show top 5
#                 # Use the correct field names from API response
#                 product_name = item.get('product_name_en', item.get('name', 'Unknown Product'))
#                 product_id = item.get('product_id', item.get('id', 'N/A'))
#                 score = item.get('score', item.get('similarity_score', 'N/A'))
#                 description = item.get('product_description_en', item.get('description', ''))
                
#                 formatted += f"{i}. **{product_name}**\n"
#                 formatted += f"   Product ID: {product_id}\n"
#                 formatted += f"   Similarity Score: {score:.2f if isinstance(score, (int, float)) else score}\n"
#                 if description:
#                     # Truncate long descriptions
#                     short_desc = description[:100] + "..." if len(description) > 100 else description
#                     formatted += f"   Description: {short_desc}\n"
#                 formatted += "\n"
#             return formatted
#         else:
#             return "No similar products found."
    
#     return "Unable to format recommendations."

# # --- Translation function for existing messages ---
# def translate_content(text, from_lang, to_lang):
#     """Translation function that uses both keyword mapping and AI translation."""
#     if from_lang == to_lang:
#         return text
    
#     # Define translation mappings for quick common phrases
#     translations = {
#         ("id", "en"): {
#             "Kenapa daun sawit saya kuning?": "Why are my palm leaves yellow?",
#             "Bagaimana cara pupuk sawit yang bagus?": "What's a good way to fertilize palm oil trees?",
#             "Apa hama yang sering menyerang sawit?": "What pests commonly attack palm oil plants?",
#             "Bagaimana panen sawit yang efektif?": "How to harvest palm oil effectively?"
#         },
#         ("en", "id"): {
#             "Why are my palm leaves yellow?": "Kenapa daun sawit saya kuning?",
#             "What's a good way to fertilize palm oil trees?": "Bagaimana cara pupuk sawit yang bagus?",
#             "What pests commonly attack palm oil plants?": "Apa hama yang sering menyerang sawit?",
#             "How to harvest palm oil effectively?": "Bagaimana panen sawit yang efektif?"
#         }
#     }
    
#     # Get the appropriate translation dictionary
#     trans_dict = translations.get((from_lang, to_lang), {})
    
#     # Try exact match first for common phrases
#     if text.strip() in trans_dict:
#         return trans_dict[text.strip()]
    
#     # Use AI translation for longer or complex text
#     try:
#         target_language = "English" if to_lang == "en" else "Indonesian"
#         source_language = "Indonesian" if from_lang == "id" else "English"
        
#         translation_prompt = f"""Translate the following text from {source_language} to {target_language}. 
#         This is about palm oil farming. Provide only the translation, no explanations:

#         Text to translate: {text}
        
#         Translation:"""
        
#         response = client.chat_completion(
#             model="meta-llama/Llama-3.2-3B-Instruct",
#             messages=[{"role": "user", "content": translation_prompt}],
#             temperature=0.3,
#             max_tokens=300,
#             top_p=0.9
#         )
        
#         translated_text = response.choices[0].message.content.strip()
        
#         # Clean up the response - remove any extra text
#         if "Translation:" in translated_text:
#             translated_text = translated_text.split("Translation:")[-1].strip()
#         if translated_text.startswith('"') and translated_text.endswith('"'):
#             translated_text = translated_text[1:-1]
        
#         return translated_text
        
#     except Exception as e:
#         # Fallback to keyword mapping if AI translation fails
#         translated_text = text
#         sorted_translations = sorted(trans_dict.items(), key=lambda x: len(x[0]), reverse=True)
        
#         for original, translation in sorted_translations:
#             if original.lower() in translated_text.lower():
#                 import re
#                 pattern = re.compile(re.escape(original), re.IGNORECASE)
#                 translated_text = pattern.sub(translation, translated_text)
        
#         return translated_text

# # --- Language Selection ---
# if "language" not in st.session_state:
#     st.session_state.language = "id"  # Default: Bahasa Indonesia

# # --- Session State Initialization ---
# if 'messages' not in st.session_state:
#     st.session_state.messages = []

# if 'current_mode' not in st.session_state:
#     st.session_state.current_mode = "general"  # Default mode

# if 'waiting_for_input' not in st.session_state:
#     st.session_state.waiting_for_input = None  # None, "user_id", "product_id"

# # --- Text Translations ---
# TEXT = {
#     "id": {
#         "title": "🌴 PalmPal – Teman Berkebun Kamu",
#         "subtitle": "*Tanya soal sawit, kapan saja, di mana saja.*",
#         "input_placeholder": "Tulis pertanyaanmu di sini (contoh: Kenapa daun sawit saya kuning?)",
#         "send_button": "📨 Kirim",
#         "examples": "Contoh Pertanyaan:",
#         "advanced": "⚙️ Pengaturan Lanjutan",
#         "clear_chat": "🧹 Bersihkan Riwayat Chat",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Didukung oleh AI & Llama3 💡",
#         "thinking": "PalmPal sedang berpikir...",
#         "user": "Kamu",
#         "bot": "PalmPal",
#         "error": "Maaf, PalmPal mengalami gangguan.",
#         "lang_toggle": "🌐 Switch to English",
#         "mode_user": "👤 Rekomendasi Pengguna",
#         "mode_product": "📦 Rekomendasi Produk", 
#         "mode_general": "💬 Chat Umum",
#         "current_mode_text": "Mode saat ini:",
#         "enter_user_id": "Masukkan User ID Anda:",
#         "enter_product_id": "Masukkan Product ID:",
#         "mode_selection": "Pilih mode yang ingin Anda gunakan:"
#     },
#     "en": {
#         "title": "🌴 PalmPal – Your Farming Friend",
#         "subtitle": "*Ask anything about palm oil, anytime, anywhere.*",
#         "input_placeholder": "Type your question here (e.g. Why are my palm leaves yellow?)",
#         "send_button": "📨 Send",
#         "examples": "Example Questions:",
#         "advanced": "⚙️ Advanced Settings",
#         "clear_chat": "🧹 Clear Chat History",
#         "model_info": "Model: `meta-llama/Llama-3.2-3B-Instruct` via HuggingFace API",
#         "footer": "© 2025 SawitPRO – Powered by AI & Llama3 💡",
#         "thinking": "PalmPal is thinking...",
#         "user": "You",
#         "bot": "PalmPal",
#         "error": "Sorry, PalmPal encountered an error.",
#         "lang_toggle": "🌐 Ganti ke Bahasa Indonesia",
#         "mode_user": "👤 User Recommendations",
#         "mode_product": "📦 Product Recommendations",
#         "mode_general": "💬 General Chat",
#         "current_mode_text": "Current mode:",
#         "enter_user_id": "Enter your User ID:",
#         "enter_product_id": "Enter Product ID:",
#         "mode_selection": "Choose the mode you want to use:"
#     }
# }
# lang = st.session_state.language
# t = TEXT[lang]

# # --- Language Toggle Button with AI Translation Feature ---
# st.markdown('<div class="lang-toggle">', unsafe_allow_html=True)
# lang_toggle = st.button(t["lang_toggle"])
# if lang_toggle:
#     # Store the old language before switching
#     old_lang = st.session_state.language
#     new_lang = "en" if st.session_state.language == "id" else "id"
    
#     # Translate existing messages when language is switched
#     if 'messages' in st.session_state and st.session_state.messages:
#         with st.spinner("Translating messages..." if new_lang == "en" else "Menerjemahkan pesan..."):
#             translated_messages = []
#             for message in st.session_state.messages:
#                 original_content = message["content"]
#                 translated_content = translate_content(original_content, old_lang, new_lang)
#                 translated_messages.append({
#                     "role": message["role"],
#                     "content": translated_content
#                 })
            
#             st.session_state.messages = translated_messages
    
#     # Switch the language
#     st.session_state.language = new_lang
#     st.rerun()
# st.markdown('</div>', unsafe_allow_html=True)

# # --- Branding ---
# st.image("https://static.wixstatic.com/media/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png/v1/fit/w_2500,h_1330,al_c/759737_bb3e5ad0e411479782bdaca6f4fa3bda~mv2.png", width=120)
# st.markdown(f"## {t['title']}")
# st.markdown(t['subtitle'])

# # --- Mode Selection Buttons ---
# st.markdown(f"**{t['mode_selection']}**")
# st.markdown('<div class="mode-grid">', unsafe_allow_html=True)

# col1, col2, col3 = st.columns(3)
# with col1:
#     if st.button(t["mode_user"], key="mode_user_btn"):
#         st.session_state.current_mode = "user"
#         st.session_state.waiting_for_input = "user_id"
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": t["enter_user_id"]
#         })
#         st.rerun()

# with col2:
#     if st.button(t["mode_product"], key="mode_product_btn"):
#         st.session_state.current_mode = "product"
#         st.session_state.waiting_for_input = "product_id"
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": t["enter_product_id"]
#         })
#         st.rerun()

# with col3:
#     if st.button(t["mode_general"], key="mode_general_btn"):
#         st.session_state.current_mode = "general"
#         st.session_state.waiting_for_input = None
#         if lang == "id":
#             welcome_msg = "Selamat datang di mode chat umum! Tanyakan apa saja tentang pertanian sawit."
#         else:
#             welcome_msg = "Welcome to general chat mode! Ask anything about palm oil farming."
#         st.session_state.messages.append({
#             "role": "assistant", 
#             "content": welcome_msg
#         })
#         st.rerun()

# st.markdown('</div>', unsafe_allow_html=True)

# # --- Current Mode Indicator ---
# mode_names = {
#     "user": t["mode_user"],
#     "product": t["mode_product"], 
#     "general": t["mode_general"]
# }
# st.markdown(f'<div class="current-mode">{t["current_mode_text"]} {mode_names[st.session_state.current_mode]}</div>', unsafe_allow_html=True)

# # --- Generate Bot Response ---
# def generate_response(user_input):
#     messages = []
#     for msg in st.session_state.messages[-10:]:
#         role = "user" if msg["role"] == "user" else "assistant"
#         messages.append({"role": role, "content": msg["content"]})
#     messages.append({"role": "user", "content": user_input})

#     with st.spinner(t['thinking']):
#         try:
#             response = client.chat_completion(
#                 model="meta-llama/Llama-3.2-3B-Instruct",
#                 messages=messages,
#                 temperature=0.7,
#                 max_tokens=500,
#                 top_p=0.9
#             )
#             return response.choices[0].message.content
#         except Exception as e:
#             st.error(f"{t['error']}: {str(e)}")
#             return t['error']

# # --- Process API Requests ---
# def process_api_request(user_input):
#     """Process user input based on current waiting state"""
#     if st.session_state.waiting_for_input == "user_id":
#         # User provided user ID
#         with st.spinner("Fetching user recommendations..." if lang == "en" else "Mengambil rekomendasi pengguna..."):
#             success, data = get_user_recommendations(user_input.strip())
            
#             if success:
#                 formatted_response = format_recommendations(data, "user")
#                 # Add option to return to mode selection
#                 if lang == "id":
#                     formatted_response += "\n\n✨ Rekomendasi selesai! Silakan pilih mode lagi atau lanjutkan chat umum."
#                 else:
#                     formatted_response += "\n\n✨ Recommendations complete! Please select a mode again or continue with general chat."
#             else:
#                 formatted_response = data  # Error message
                
#         st.session_state.waiting_for_input = None
#         st.session_state.current_mode = "general"
#         return formatted_response
        
#     elif st.session_state.waiting_for_input == "product_id":
#         # User provided product ID  
#         with st.spinner("Fetching product recommendations..." if lang == "en" else "Mengambil rekomendasi produk..."):
#             success, data = get_product_recommendations(user_input.strip())
            
#             if success:
#                 formatted_response = format_recommendations(data, "product")
#                 # Add option to return to mode selection
#                 if lang == "id":
#                     formatted_response += "\n\n✨ Rekomendasi selesai! Silakan pilih mode lagi atau lanjutkan chat umum."
#                 else:
#                     formatted_response += "\n\n✨ Recommendations complete! Please select a mode again or continue with general chat."
#             else:
#                 formatted_response = data  # Error message
                
#         st.session_state.waiting_for_input = None
#         st.session_state.current_mode = "general"
#         return formatted_response
    
#     else:
#         # General chat mode
#         return generate_response(user_input)

# # --- Display Chat Bubbles ---
# for msg in st.session_state.messages[-10:]:
#     if msg["role"] == "user":
#         st.markdown(f"""
#         <div class="chat-container user">
#             <div class="chat-bubble user-bubble">{msg['content']}</div>
#             <div class="chat-icon user-icon"></div>
#         </div>
#         """, unsafe_allow_html=True)
#     else:
#         st.markdown(f"""
#         <div class="chat-container bot">
#             <div class="chat-icon bot-icon"></div>
#             <div class="chat-bubble bot-bubble">{msg['content']}</div>
#         </div>
#         """, unsafe_allow_html=True)

# # --- Example Prompts (only show for general mode) ---
# if st.session_state.current_mode == "general" and st.session_state.waiting_for_input is None:
#     st.markdown(f'<p class="example-header">{t["examples"]}</p>', unsafe_allow_html=True)

#     example_prompts = {
#         "id": [
#             "Kenapa daun sawit saya kuning?",
#             "Bagaimana cara pupuk sawit yang bagus?",
#             "Apa hama yang sering menyerang sawit?",
#             "Bagaimana panen sawit yang efektif?"
#         ],
#         "en": [
#             "Why are my palm leaves yellow?",
#             "What's a good way to fertilize palm oil trees?",
#             "What pests commonly attack palm oil plants?",
#             "How to harvest palm oil effectively?"
#         ]
#     }

#     # Custom HTML grid for examples with equal spacing
#     st.markdown('<div class="example-grid">', unsafe_allow_html=True)
#     for i, prompt in enumerate(example_prompts[lang]):
#         if st.button(prompt, key=f"example_{i}"):
#             st.session_state.messages.append({"role": "user", "content": prompt})
#             bot_response = generate_response(prompt)
#             st.session_state.messages.append({"role": "assistant", "content": bot_response})
#             st.rerun()
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- User Input ---
# def process_user_input():
#     if st.session_state.user_input:
#         user_message = st.session_state.user_input
#         st.session_state.messages.append({"role": "user", "content": user_message})
        
#         # Process based on current mode and waiting state
#         bot_response = process_api_request(user_message)
#         st.session_state.messages.append({"role": "assistant", "content": bot_response})
#         st.session_state.user_input = ""  # Clear input field

# # Input area with better alignment
# col1, col2 = st.columns([4, 1])
# with col1:
#     placeholder_text = t["input_placeholder"]
#     if st.session_state.waiting_for_input == "user_id":
#         placeholder_text = t["enter_user_id"]
#     elif st.session_state.waiting_for_input == "product_id":
#         placeholder_text = t["enter_product_id"]
        
#     # Remove on_change to prevent double sending
#     st.text_input(placeholder_text, key="user_input")
# with col2:
#     st.markdown('<div class="send-button">', unsafe_allow_html=True)
#     st.button(t["send_button"], on_click=process_user_input)
#     st.markdown('</div>', unsafe_allow_html=True)

# # --- Optional Advanced Section ---
# with st.expander(t["advanced"]):
#     if st.button(t["clear_chat"]):
#         st.session_state.messages = []
#         st.session_state.current_mode = "general"
#         st.session_state.waiting_for_input = None
#         st.rerun()
#     st.markdown(t["model_info"])
